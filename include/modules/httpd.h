...
class HTTPQueryParameters : public
    insp::flat_multimap<std::string, std::string> {
  public:
...
class HTTPHeaders {
  ...
  public:
    ...
    std::string GetFormattedHeaders() {
        ...
        for (std::map<std::string,std::string>::iterator i = headers.begin();
                i != headers.end(); i++) {
            re += i->first + ": " + i->second + "\r\n";
        }
...
class HTTPRequest {
  ...
  public:
     ...
     */
    HTTPRequest(const std::string& request_type, const HTTPRequestURI& parsed_uri,
                HTTPHeaders* hdr,
...
class HTTPDocumentResponse {
  public:
     ...
     */
    HTTPDocumentResponse(Module* mod, HTTPRequest& req, std::stringstream* doc,
