...
class CoreExport Module : public classbase, public usecountbase {
  ...
  public:
     ...
     */
    virtual void OnUserJoin(Membership* memb, bool sync, bool created,
                            CUList& except_list);
     ...
     */
    virtual void OnUserPart(Membership* memb, std::string &partmessage,
                            CUList& except_list);
     ...
     */
    virtual ModResult OnSendSnotice(char &snomask, std::string &type,
     ...
     */
    virtual ModResult OnUserPreJoin(LocalUser* user, Channel* chan,
                                    const std::string& cname, std::string& privs, const std::string& keygiven);
     ...
     */
    virtual ModResult OnUserPreKick(User* source, Membership* memb,
     ...
     * @param except_list A list of users to not send to.
     */
    virtual void OnUserKick(User* source, Membership* memb,
     ...
     * @param opertype The opers type name
     */
    virtual void OnPostOper(User* user, const std::string &opername,
     ...
     */
    virtual ModResult OnUserPreInvite(User* source,User* dest,Channel* channel,
                                      time_t timeout);
    ...
    virtual void OnUserInvite(User* source, User* dest, Channel* channel,
                              time_t timeout, unsigned int notifyrank, CUList& notifyexcepts);
     ...
     */
    virtual ModResult OnUserPreMessage(User* user, const MessageTarget& target,
     ...
     */
    virtual void OnBuildNeighborList(User* source, IncludeChanList& include_c,
     ...
     *                MessageDetails class for more information.
     */
    virtual void OnUserPostMessage(User* user, const MessageTarget& target,
     ...
     *                MessageDetails class for more information.
     */
    virtual void OnUserMessage(User* user, const MessageTarget& target,
                               const MessageDetails& details);
     ...
     *                MessageDetails class for more information.
     */
    virtual void OnUserMessageBlocked(User* user, const MessageTarget& target,
                                      const MessageDetails& details);
     ...
     */
    virtual void OnMode(User* user, User* usertarget, Channel* chantarget,
                        const Modes::ChangeList& changelist, ModeParser::ModeProcessFlag processflags);
     ...
     */
    virtual void OnDecodeMetaData(Extensible* target, const std::string &extname,
     ...
     */
    virtual ModResult OnPreMode(User* source, User* dest, Channel* channel,
    ...
    */
     ...
     */
    virtual ModResult OnPreCommand(std::string& command,
     ...
     */
    virtual void OnPostCommand(Command* command,
     ...
     * @param user The user issuing the command.
     */
    virtual void OnCommandBlocked(const std::string& command,
                                  const CommandBase::Params& parameters, LocalUser* user);
     ...
     * will be ignored!
     */
    virtual ModResult OnRawMode(User* user, Channel* chan, ModeHandler* mh,
                                const std::string& param, bool adding);
     ...
     * @return 1 to explicitly allow the join, 0 to proceed as normal
     */
    virtual ModResult OnCheckKey(User* user, Channel* chan,
     ...
     */
    virtual ModResult OnPreChangeRealName(LocalUser* user,
     ...
     */
    virtual ModResult OnPreTopicChange(User* user, Channel* chan,
     ...
     * @param topic The actual topic text
     */
    virtual void OnPostTopicChange(User* user, Channel* chan,
                                   const std::string &topic);
     ...
     */
    virtual ModResult OnPassCompare(Extensible* ex, const std::string &password,
     ...
     */
    virtual ModResult OnAcceptConnection(int fd, ListenSocket* sock,
...
class CoreExport ModuleManager : public fakederef<ModuleManager> {
  public:
    typedef std::multimap<std::string, ServiceProvider*, irc::insensitive_swo>
    DataProviderMap;
  ...
  public:
     ...
     * or after.
     */
    bool SetPriority(Module* mod, Implementation i, Priority s,
                     Module* which = NULL);
