...
class IOHookProvider : public refcountbase, public ServiceProvider {
  ...
  public:
     ...
     */
    IOHookProvider(Module* mod, const std::string& Name,
                   Type hooktype = IOH_UNKNOWN, bool middle = false)
        : ServiceProvider(mod, Name, SERVICE_IOHOOK), middlehook(middle),
          type(hooktype) { }
     ...
     */
    virtual void OnAccept(StreamSocket* sock, irc::sockets::sockaddrs* client,
                          irc::sockets::sockaddrs* server) = 0;
...
class IOHook : public classbase {
  public:
    ...
    virtual bool IsHookReady() const {
        return true;
    }
     ...
     */
    virtual int OnStreamSocketWrite(StreamSocket* sock,
                                    StreamSocket::SendQueue& sendq) = 0;
