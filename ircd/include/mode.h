...
class CoreExport ModeHandler : public ServiceProvider {
  ...
  public:
     ...
     */
    ModeHandler(Module* me, const std::string& name, char modeletter,
                ParamSpec params, ModeType type, Class mclass = MC_OTHER);
     ...
     */
    virtual ModResult AccessCheck(User* source, Channel* channel,
     ...
     */
    virtual ModeAction OnModeChange(User* source, User* dest, Channel* channel,
                                    std::string &parameter, bool
                                    adding); /* Can change the mode parameter as its a ref */
     ...
     */
    virtual void OnParameterInvalid(User* user, Channel* targetchannel,
     ...
     */
    virtual bool ResolveModeConflict(std::string &their_param,
                                     const std::string &our_param, Channel* channel);
...
class CoreExport PrefixMode : public ModeHandler {
  ...
  public:
    ...
    PrefixMode(Module* Creator, const std::string& Name, char ModeLetter,
               unsigned int Rank = 0, char PrefixChar = 0);
     ...
     */
    void Update(unsigned int rank, unsigned int setrank, unsigned int unsetrank,
                bool selfrm);
...
class CoreExport SimpleChannelModeHandler : public ModeHandler {
  public:
    SimpleChannelModeHandler(Module* Creator, const std::string& Name,
                             char modeletter, bool operonly = false)
...
class CoreExport ModeWatcher : public classbase {
  ...
  public:
     ...
     */
    virtual bool BeforeMode(User* source, User* dest, Channel* channel,
                            std::string& parameter, bool adding);
    /**
     ...
     * @param adding True if the mode is being added and false if it is being removed
     */
    virtual void AfterMode(User* source, User* dest, Channel* channel,
                           const std::string& parameter, bool adding);
};
...
class CoreExport ModeParser : public fakederef<ModeParser> {
  public:
     ...
     */
    typedef TR1NS::unordered_map<std::string, ModeHandler*, irc::insensitive, irc::StrHashComp>
    ModeHandlerMap;
  ...
  private:
     ...
     */
    ModeAction TryMode(User* user, User* usertarget, Channel* chantarget,
  ...
  public:
     ...
     */
    void Process(User* user, Channel* targetchannel, User* targetuser,
     ...
     */
    unsigned int ProcessSingle(User* user, Channel* targetchannel, User* targetuser,
                               Modes::ChangeList& changelist, ModeProcessFlag flags = MODE_NONE,
     ...
     */
    void ModeParamsToChangeList(User* user, ModeType type,
                                const std::vector<std::string>& parameters, Modes::ChangeList& changelist,
                                unsigned int beginindex = 1, unsigned int endindex = UINT_MAX);
...
inline const PrefixMode* ModeHandler::IsPrefixMode() const {
    return (this->type_id == MC_PREFIX ? static_cast<const PrefixMode*>
            (this) : NULL);
}

inline ListModeBase* ModeHandler::IsListModeBase() {
    return (this->type_id == MC_LIST ? reinterpret_cast<ListModeBase*>
            (this) : NULL);
}

inline const ListModeBase* ModeHandler::IsListModeBase() const {
    return (this->type_id == MC_LIST ? reinterpret_cast<const ListModeBase*>
            (this) : NULL);
}

inline ParamModeBase* ModeHandler::IsParameterMode() {
    return (this->type_id == MC_PARAM ? reinterpret_cast<ParamModeBase*>
            (this) : NULL);
}

inline const ParamModeBase* ModeHandler::IsParameterMode() const {
    return (this->type_id == MC_PARAM ? reinterpret_cast<const ParamModeBase*>
            (this) : NULL);
}
