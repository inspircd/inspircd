...
class CoreExport StreamSocket : public EventHandler {
  ...
  public:
     ...
     */
    virtual bool OnSetEndPoint(const irc::sockets::sockaddrs& local,
...
class CoreExport BufferedSocket : public StreamSocket {
  public:
     ...
     */
    void DoConnect(const irc::sockets::sockaddrs& dest,
  ...
  protected:
    void OnEventHandlerWrite() CXX11_OVERRIDE;
    BufferedSocketError BeginConnect(const irc::sockets::sockaddrs& dest,
                                     const irc::sockets::sockaddrs& bind, unsigned int timeout);
};

inline IOHook* StreamSocket::GetIOHook() const {
    return iohook;
}
inline void StreamSocket::DelIOHook() {
    iohook = NULL;
}
