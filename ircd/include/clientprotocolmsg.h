...
class CoreExport ClientProtocol::Messages::Numeric : public
    ...
    void InitFromNumeric(const ::Numeric::Numeric& numeric) {
        ...
        for (std::vector<std::string>::const_iterator i = numeric.GetParams().begin();
                i != numeric.GetParams().end(); ++i) {
            PushParamRef(*i);
        }
    }
  ...
  public:
    ...
    Numeric(const ::Numeric::Numeric& num, User* user)
        : ClientProtocol::Message(NULL,
                                  (num.GetServer() ? num.GetServer() :
                                   ServerInstance->FakeClient->server)->GetPublicName()) {
 ...
 */
class CoreExport ClientProtocol::Messages::Join : public
    ClientProtocol::Message {
    Membership* memb;
 ...
 */
struct CoreExport ClientProtocol::Messages::Part : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
struct CoreExport ClientProtocol::Messages::Kick : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
struct CoreExport ClientProtocol::Messages::Quit : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
struct CoreExport ClientProtocol::Messages::Nick : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
class CoreExport ClientProtocol::Messages::Mode : public
    ClientProtocol::Message {
    Channel* chantarget;
    User* usertarget;
     ...
     */
    static std::string ToModeLetters(const Modes::ChangeList::List& list,
                                     std::string::size_type maxlinelen,
                                     Modes::ChangeList::List::const_iterator beginit,
                                     Modes::ChangeList::List::const_iterator& lastit) {
        std::string ret;
  ...
  public:
    ...
    static std::string ToModeLetters(const Modes::ChangeList& changelist) {
        ...
        return ToModeLetters(changelist.getlist(), UINT_MAX,
                             changelist.getlist().begin(), dummy);
    }
     ...
     * @param changelist Mode change list. Must remain valid and unchanged as long as this object is alive or until the next SetParams() call.
     */
    void SetParams(Channel* Chantarget, User* Usertarget,
                   const Modes::ChangeList& changelist) {
        ClearParams();
    ...
    const std::string& GetStrTarget() const {
        return (chantarget ? chantarget->name : usertarget->nick);
    }
...
/** TOPIC message.
 */
struct CoreExport ClientProtocol::Messages::Topic : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
class CoreExport ClientProtocol::Messages::Privmsg : public
    ClientProtocol::Message {
  ...
  public:
     ...
     */
    Privmsg(User* source, const std::string& target, const std::string& text,
     ...
     */
    Privmsg(User* source, const Channel* targetchan, const std::string& text,
            MessageType mt = MSG_PRIVMSG, char status = 0)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
     ...
     * @param mt Message type.
     */
    Privmsg(User* source, const User* targetuser, const std::string& text,
            MessageType mt = MSG_PRIVMSG)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
     ...
     * @param status Prefix character for status messages. If non-zero the message is a status message. Optional, defaults to 0.
     */
    Privmsg(const std::string& source, const std::string& target,
            const std::string& text, MessageType mt = MSG_PRIVMSG, char status = 0)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        if (status) {
            std::string rawtarget(1, status);
     ...
     * @param mt Message type.
     */
    Privmsg(const std::string& source, const Channel* targetchan,
            const std::string& text, MessageType mt = MSG_PRIVMSG, char status = 0)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushTargetChan(status, targetchan);
        PushParam(text);
     ...
     * @param mt Message type.
     */
    Privmsg(const std::string& source, const User* targetuser,
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, User* source, const std::string& target,
            const std::string& text, MessageType mt = MSG_PRIVMSG)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushParam(target);
        PushParamRef(text);
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, User* source, const Channel* targetchan,
            const std::string& text, MessageType mt = MSG_PRIVMSG, char status = 0)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushTargetChan(status, targetchan);
        PushParamRef(text);
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, User* source, const User* targetuser, const std::string& text,
            MessageType mt = MSG_PRIVMSG)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushTargetUser(targetuser);
        PushParamRef(text);
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, const std::string& source, const std::string& target,
            const std::string& text, MessageType mt = MSG_PRIVMSG)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushParam(target);
        PushParamRef(text);
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, const std::string& source, const Channel* targetchan,
            const std::string& text, MessageType mt = MSG_PRIVMSG, char status = 0)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushTargetChan(status, targetchan);
        PushParamRef(text);
     ...
     * @param mt Message type.
     */
    Privmsg(NoCopy, const std::string& source, const User* targetuser,
            const std::string& text, MessageType mt = MSG_PRIVMSG)
        : ClientProtocol::Message(CommandStrFromMsgType(mt), source) {
        PushTargetUser(targetuser);
        PushParamRef(text);
 ...
 */
struct CoreExport ClientProtocol::Messages::Invite : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
struct CoreExport ClientProtocol::Messages::Ping : public
    ClientProtocol::Message {
    /** Constructor.
 ...
 */
struct CoreExport ClientProtocol::Messages::Pong : public
    ClientProtocol::Message {
    /** Constructor.
     * @param cookie Ping cookie. Must remain valid as long as this object is alive.
 ...
 */
struct CoreExport ClientProtocol::Messages::Error : public
    ClientProtocol::Message {
    /** Constructor.
