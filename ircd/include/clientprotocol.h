 ...
 */
class CoreExport ClientProtocol::Message : public
  ...
  private:
    typedef std::vector<std::pair<SerializedInfo, SerializedMessage> >
    SerializedList;
  ...
  public:
     ...
     */
    void AddTag(const std::string& tagname, MessageTagProvider* tagprov,
                const std::string& val, void* tagdata = NULL) {
     ...
     */
    const SerializedMessage& GetSerialized(const SerializedInfo& serializeinfo)
    const;
 ...
 */
class CoreExport ClientProtocol::MessageTagProvider : public
  ...
  public:
     ...
     */
    virtual ModResult OnProcessTag(User* user, const std::string& tagname,
                                   std::string& tagvalue) {
 ...
 */
class CoreExport ClientProtocol::EventHook : public
    Events::ModuleEventListener {
  public:
     ...
     * Optional.
     */
    EventHook(Module* mod, const std::string& name,
     ...
     */
    virtual ModResult OnPreEventSend(LocalUser* user,
                                     const ClientProtocol::Event& ev, ClientProtocol::MessageList& messagelist) = 0;
};
 ...
 */
class CoreExport ClientProtocol::EventProvider : public
  ...
  public:
    ...
    EventProvider(Module* Mod, const std::string& eventname)
        : Events::ModuleEventProvider(Mod,
                                      ClientProtocol::EventHook::GetEventName(eventname)) {
    }
};
...
class CoreExport ClientProtocol::Serializer : public DataProvider {
  ...
  public:
     ...
     */
    bool HandleTag(LocalUser* user, const std::string& tagname,
                   std::string& tagvalue, TagMap& tags) const;
    ...
    virtual std::string Serialize(const Message& msg,
                                  const TagSelection& tagwl) const = 0;
     ...
     */
    virtual bool Parse(LocalUser* user, const std::string& line,
                       ParseOutput& parseoutput) = 0;
};

inline ClientProtocol::MessageTagData::MessageTagData(MessageTagProvider* prov,
        const std::string& val, void* data)
    : tagprov(prov)
    , value(val)
    , provdata(data) {
}
