...
bool InspIRCd::PassCompare(Extensible* ex, const std::string& data,
...
bool CommandParser::LoopCall(User* user, Command* handler,
                             const CommandBase::Params& parameters, unsigned int splithere, int extra,
                             bool usemax) {
    ...
    while (items1.GetToken(item) && (!usemax
                                     || max++ < ServerInstance->Config->MaxTargets)) {
...
CmdResult CommandParser::CallHandler(const std::string& commandname,
                                     const CommandBase::Params& parameters, User* user, Command** cmd) {
    /* find the command, check it exists */
    Command* handler = GetHandler(commandname);
    if (handler) {
        if ((!parameters.empty()) && (parameters.back().empty())
                && (!handler->allow_empty_last_param)) {
            return CMD_INVALID;
        }
...
void CommandParser::ProcessCommand(LocalUser* user, std::string& command,
    ...
    if (handler->max_params && command_p.size() > handler->max_params) {
        ...
        const CommandBase::Params::iterator lastkeep = command_p.begin() +
                (handler->max_params - 1);
        ...
        for (CommandBase::Params::const_iterator i = firstexcess; i != command_p.end();
                ++i) {
            lastkeep->push_back(' ');
            lastkeep->append(*i);
        }
    ...
    if (handler->flags_needed) {
        ...
        if (!user->HasCommandPermission(command)) {
            user->CommandFloodPenalty += failpenalty;
            user->WriteNumeric(ERR_NOPRIVILEGES,
                               InspIRCd::Format("Permission Denied - Oper type %s does not have access to command %s",
    ...
    if ((!command_p.empty()) && (command_p.back().empty())
            && (!handler->allow_empty_last_param)) {
        command_p.pop_back();
    }
...
std::string CommandParser::TranslateUIDs(const std::vector<TranslateType>& to,
        const CommandBase::Params& source, bool prefix_final,
        CommandBase* custom_translator) {
...
void CommandParser::TranslateSingleParam(TranslateType to,
        const std::string& item, std::string& dest, CommandBase* custom_translator,
