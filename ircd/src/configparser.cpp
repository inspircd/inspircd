...
struct Parser {
    ...
    Parser(ParseStack& me, int myflags, FILE* conf, const std::string& name,
           const std::string& mandatorytag)
        : stack(me), flags(myflags), file(conf), current(name), last_tag(name),
          ungot(-1), mandatory_tag(mandatorytag)
    { }
    ...
    bool kv(ConfigItems* items) {
        ...
        if (ch == '>' && key.empty()) {
            ...
            throw CoreException("Invalid character " + std::string(1,
                                ch) + " in key (" + key + ")");
        }
        ...
        while (1) {
            ...
            if (ch == '&' && !(flags & FLAG_USE_COMPAT)) {
                ...
                if (varname.empty()) {
                ...
                } else if (varname[0] == '#' && (varname.size() == 1 || (varname.size() == 2
                                                 && varname[1] == 'x'))) {
                    ...
                    insp::flat_map<std::string, std::string>::iterator var = stack.vars.find(
                                varname);
...
void ParseStack::DoInclude(ConfigTag* tag, int flags) {
    ...
    if (tag->readString("file", name)) {
        ...
        if (!ParseFile(ServerInstance->Config->Paths.PrependConfig(name), flags,
                       mandatorytag)) {
        ...
        const std::string includedir = ServerInstance->Config->Paths.PrependConfig(
                                           name);
...
void CheckOwnership(const std::string& path) {
    ...
    if (getegid() != pathinfo.st_gid) {
        ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                  "Possible configuration error: %s is owned by group %u but the server is running as group %u.",
                                  path.c_str(), pathinfo.st_gid, getegid());
    }

    if (geteuid() != pathinfo.st_uid) {
        ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                  "Possible configuration error: %s is owned by user %u but the server is running as user %u.",
...
void ParseStack::DoReadFile(const std::string& key, const std::string& name,
                            int flags, bool exec) {
    if (flags & FLAG_NO_INC) {
    ...
    FileWrapper file(exec ? popen(name.c_str(), "r") : fopen(path.c_str(), "r"),
                     exec);
    if (!file) {
        throw CoreException("Could not read \"" + path + "\" for \"" + key +
                            "\" file");
    }
...
bool ParseStack::ParseFile(const std::string& path, int flags,
                           const std::string& mandatory_tag, bool isexec) {
    ServerInstance->Logs->Log("CONFIG", LOG_DEBUG, "Reading (isexec=%d) %s", isexec,
                              path.c_str());
    if (stdalgo::isin(reading, path)) {
        throw CoreException((isexec ? "Executable " : "File ") + path +
    ...
    FileWrapper file((isexec ? popen(path.c_str(), "r") : fopen(path.c_str(), "r")),
                     isexec);
    if (!file) {
...
bool ConfigTag::readString(const std::string& key, std::string& value,
                           bool allow_lf) {
    for(ConfigItems::iterator j = items.begin(); j != items.end(); ++j) {
        ...
        if (!allow_lf && (value.find('\n') != std::string::npos)) {
            ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                      "Value of <" + tag + ":" + key + "> at " + getTagLocation() +
...
std::string ConfigTag::getString(const std::string& key, const std::string& def,
    ...
    if (!validator(res)) {
        ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                  "WARNING: The value of <%s:%s> is not valid; value set to %s.",
...
std::string ConfigTag::getString(const std::string& key, const std::string& def,
    ...
    if (res.length() < minlen || res.length() > maxlen) {
        ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                  "WARNING: The length of <%s:%s> is not between %ld and %ld; value set to %s.",
                                  tag.c_str(), key.c_str(), (unsigned long)minlen, (unsigned long)maxlen,
                                  def.c_str());
        return def;
    }
    return res;
...
void CheckMagnitude(const std::string& tag, const std::string& key,
                    const std::string& val, Numeric& num, Numeric def, const char* tail) {
    ...
    const std::string message = "WARNING: <" + tag + ":" + key + "> value of " + val
                                + " contains an invalid magnitude specifier '"
 ...
 */
template <typename Numeric>
void CheckRange(const std::string& tag, const std::string& key, Numeric& num,
                Numeric def, Numeric min, Numeric max) {
    if (num >= min && num <= max) {
        return;
    }

    const std::string message = "WARNING: <" + tag + ":" + key + "> value of " +
                                ConvToStr(num) + " is not between "
                                + ConvToStr(min) + " and " + ConvToStr(max) + "; value set to " + ConvToStr(
                                    def) + ".";
    ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT, message);
    num = def;
}
...
unsigned long ConfigTag::getUInt(const std::string& key, unsigned long def,
...
unsigned long ConfigTag::getDuration(const std::string& key, unsigned long def,
                                     unsigned long min, unsigned long max) {
    std::string duration;
...
double ConfigTag::getFloat(const std::string& key, double def, double min,
                           double max) {
    std::string result;
...
bool ConfigTag::getBool(const std::string &key, bool def) {
    ...
    if (stdalgo::string::equalsci(result, "yes")
            || stdalgo::string::equalsci(result, "true")
            || stdalgo::string::equalsci(result, "on") || result == "1") {
        return true;
    }

    if (stdalgo::string::equalsci(result, "no")
            || stdalgo::string::equalsci(result, "false")
            || stdalgo::string::equalsci(result, "off") || result == "0") {
        return false;
    }
...
unsigned char ConfigTag::getCharacter(const std::string &key,
                                      unsigned char def) {
    std::string result;
...
ConfigTag* ConfigTag::create(const std::string& Tag, const std::string& file,
