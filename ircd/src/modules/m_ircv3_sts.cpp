...
class STSCap : public Cap::Capability {
  ...
  public:
    ...
    void SetPolicy(const std::string& newhost, unsigned long duration,
                   unsigned int port, bool preload) {
        ...
        if (!irc::equals(host, newhost)) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Changing STS SNI hostname from \"%s\" to \"%s\"", host.c_str(),
                                      newhost.c_str());
            host = newhost;
            changed = true;
        }
        ...
        if (plaintextpolicy != newplaintextpolicy) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Changing plaintext STS policy from \"%s\" to \"%s\"", plaintextpolicy.c_str(),
                                      newplaintextpolicy.c_str());
        ...
        if (securepolicy != newsecurepolicy) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Changing secure STS policy from \"%s\" to \"%s\"", securepolicy.c_str(),
                                      newsecurepolicy.c_str());
...
class ModuleIRCv3STS : public Module {
  private:
    ...
    bool HasValidSSLPort(unsigned int port) {
        for (std::vector<ListenSocket*>::const_iterator iter =
                    ServerInstance->ports.begin(); iter != ServerInstance->ports.end(); ++iter) {
            ListenSocket* ls = *iter;

            // Is this listener marked as providing SSL over HAProxy?
            if (!ls->bind_tag->getString("hook").empty()
                    && ls->bind_tag->getBool("sslhook")) {
                return true;
            }
  ...
  public:
    ...
    void ReadConfig(ConfigStatus& status) CXX11_OVERRIDE {
        ...
        if (host.empty()) {
            throw ModuleException("<sts:host> must contain a hostname, at " +
                                  tag->getTagLocation());
        }

        unsigned int port = tag->getUInt("port", 6697, 1, 65535);
        if (!HasValidSSLPort(port)) {
            throw ModuleException("<sts:port> must be a TLS port, at " +
                                  tag->getTagLocation());
        }
