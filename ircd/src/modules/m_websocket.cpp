...
class WebSocketHook : public IOHookMiddle {
    class HTTPHeaderFinder {
      ...
      public:
        bool Find(const std::string& req, const char* header,
                  std::string::size_type headerlen, std::string::size_type maxpos) {
    ...
    static size_t FillHeader(unsigned char* outbuf, size_t sendlength,
                             OpCode opcode) {
        size_t pos = 0;
    ...
    static StreamSocket::SendQueue::Element PrepareSendQElem(size_t size,
            OpCode opcode) {
        unsigned char header[MAXHEADERSIZE];
        const size_t n = FillHeader(header, size, opcode);

        return StreamSocket::SendQueue::Element(reinterpret_cast<const char*>(header),
                                                n);
    }

    int HandleAppData(StreamSocket* sock, std::string& appdataout,
                      bool allowlarge) {
        ...
        if (!(len1 & WS_MASKBIT)) {
            CloseConnection(sock, CLOSE_PROTOCOL_ERROR,
                            "WebSocket protocol violation: unmasked client frame");
            return -1;
        }
        ...
        const unsigned char* maskkey = reinterpret_cast<const unsigned char*>
                                       (&cmyrecvq[2]);
        ...
        if (len1 == WS_PAYLOAD_LENGTH_MAGIC_LARGE) {
            ...
            if (!allowlarge) {
                CloseConnection(sock, CLOSE_PROTOCOL_ERROR,
                                "WebSocket protocol violation: large control frame");
                return -1;
            }
            ...
            if (len <= WS_MAX_PAYLOAD_LENGTH_SMALL) {
                CloseConnection(sock, CLOSE_PROTOCOL_ERROR,
                                "WebSocket protocol violation: non-minimal length encoding used");
                return -1;
            }
            ...
            CloseConnection(sock, CLOSE_TOO_LARGE,
                            "WebSocket: Huge frames are not supported");
            return -1;
        }
        ...
        for (std::string::const_iterator i = myrecvq.begin() + payloadstartoffset;
                i != endit; ++i) {
    ...
    int HandlePingPongFrame(StreamSocket* sock, bool isping) {
        ...
        StreamSocket::SendQueue::Element elem = PrepareSendQElem(appdata.length(),
                                                OP_PONG);
        elem.append(appdata);
    ...
    int HandleWS(StreamSocket* sock, std::string& destrecvq) {
        ...
        case OP_BINARY: {
            ...
            for (std::string::const_iterator iter = appdata.begin(); iter != appdata.end();
                    ++iter) {
                if (*iter != '\r' && *iter != '\n') {
    ...
    void CloseConnection(StreamSocket* sock, CloseCode closecode,
    ...
    void FailHandshake(StreamSocket* sock, const char* httpreply,
                       const char* sockerror) {
    ...
    int HandleHTTPReq(StreamSocket* sock) {
        ...
        if (originheader.Find(recvq, "Origin:", 7, reqend)) {
            ...
            for (WebSocketConfig::OriginList::const_iterator iter =
                        config.allowedorigins.begin(); iter != config.allowedorigins.end(); ++iter) {
        ...
        if (!config.proxyranges.empty() && sock->type == StreamSocket::SS_USER) {
            ...
            if (proxyheader.Find(recvq, "X-Real-IP:", 10, reqend)
                    || proxyheader.Find(recvq, "X-Forwarded-For:", 16, reqend)) {
            ...
            for (WebSocketConfig::ProxyRanges::const_iterator iter =
                        config.proxyranges.begin(); iter != config.proxyranges.end(); ++iter) {
                if (InspIRCd::MatchCIDR(luser->GetIPString(), *iter,
  ...
  public:
    ...
    int OnStreamSocketWrite(StreamSocket* sock,
                            StreamSocket::SendQueue& uppersendq) CXX11_OVERRIDE {
        ...
        for (StreamSocket::SendQueue::const_iterator elem = uppersendq.begin(); elem != uppersendq.end(); ++elem) {
            for (StreamSocket::SendQueue::Element::const_iterator chr = elem->begin();
                    chr != elem->end(); ++chr) {
                if (*chr == '\n') {
                    ...
                    if (sendastext) {
                        ...
                        std::string encoded;
                        utf8::unchecked::replace_invalid(message.begin(), message.end(),
    ...
    int OnStreamSocketRead(StreamSocket* sock,
                           std::string& destrecvq) CXX11_OVERRIDE {
...
void WebSocketHookProvider::OnAccept(StreamSocket* sock,
  ...
  public:
    ...
    void ReadConfig(ConfigStatus& status) CXX11_OVERRIDE {
        ...
        for (ConfigIter i = tags.first; i != tags.second; ++i) {
            ...
            if (allow.empty()) {
                throw ModuleException("<wsorigin:allow> is a mandatory field, at " +
                                      tag->getTagLocation());
            }
        ...
        if (stdalgo::string::equalsci(defaultmodestr, "reject")) {
            ...
            throw ModuleException(defaultmodestr +
                                  " is an invalid value for <websocket:defaultmode>; acceptable values are 'binary', 'text' and 'reject', at "
                                  + tag->getTagLocation());
        }
