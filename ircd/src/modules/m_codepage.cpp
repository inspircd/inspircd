...
bool IsValidNick(const std::string& nick) {
    ...
    for (std::string::const_iterator iter = nick.begin(); iter != nick.end();
            ++iter) {
    ...
    void CheckDuplicateNick() {
        ...
        for (UserManager::LocalList::const_iterator iter = list.begin();
            ...
            if (user->nick != user->uuid && !duplicates.insert(user->nick).second) {
                user->WriteNumeric(RPL_SAVENICK, user->uuid,
                                   "Your nickname is no longer available.");
    ...
    void CheckInvalidNick() {
        ...
        for (UserManager::LocalList::const_iterator iter = list.begin();
            ...
            if (user->nick != user->uuid && !ServerInstance->IsNick(user->nick)) {
                user->WriteNumeric(RPL_SAVENICK, user->uuid,
                                   "Your nickname is no longer valid.");
                user->ChangeNick(user->uuid);
            }
        }
    }
  ...
  public:
    ...
    void ReadConfig(ConfigStatus& status) CXX11_OVERRIDE {
        ...
        for (ConfigIter i = cpchars.first; i != cpchars.second; ++i) {
            ...
            unsigned char begin = tag->getUInt("begin", tag->getUInt("index", 0), 1,
                                               UCHAR_MAX);
            if (!begin) {
                throw ModuleException("<cpchars> tag without index or begin specified at " +
                                      tag->getTagLocation());
            }

            unsigned char end = tag->getUInt("end", begin, 1, UCHAR_MAX);
            if (begin > end) {
                throw ModuleException("<cpchars:begin> must be lower than <cpchars:end> at " +
                                      tag->getTagLocation());
            }
            ...
            for (unsigned short pos = begin; pos <= end; ++pos) {
                if (pos == '\n' || pos == '\r' || pos == ' ') {
                    throw ModuleException(
                        InspIRCd::Format("<cpchars> tag contains a forbidden character: %u at %s",
                                         pos, tag->getTagLocation().c_str()));
                }

                if (front && (pos == ':' || isdigit(pos))) {
                    throw ModuleException(
                        InspIRCd::Format("<cpchars> tag contains a forbidden front character: %u at %s",
                                         pos, tag->getTagLocation().c_str()));
                }
        ...
        for (ConfigIter i = cpcase.first; i != cpcase.second; ++i) {
            ...
            if (!lower) {
                throw ModuleException("<cpcase:lower> is required at " +
                                      tag->getTagLocation());
            }

            unsigned char upper = tag->getUInt("upper", 0, 1, UCHAR_MAX);
            if (!upper) {
                throw ModuleException("<cpcase:upper> is required at " +
                                      tag->getTagLocation());
            }

            newcasemap[upper] = lower;
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Marked %u (%c) as the lower case version of %u (%c)",
