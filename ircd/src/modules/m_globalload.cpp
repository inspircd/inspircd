        ...
        if (InspIRCd::Match(ServerInstance->Config->ServerName, servername)) {
            if (ServerInstance->Modules->Load(parameters[0])) {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "NEW MODULE '%s' GLOBALLY LOADED BY '%s'",parameters[0].c_str(),
                                                    user->nick.c_str());
                user->WriteRemoteNumeric(RPL_LOADEDMODULE, parameters[0],
                                         "Module successfully loaded.");
            } else {
                user->WriteRemoteNumeric(ERR_CANTLOADMODULE, parameters[0],
                                         ServerInstance->Modules->LastError());
            }
        } else {
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "MODULE '%s' GLOBAL LOAD BY '%s' (not loaded here)",parameters[0].c_str(),
                                                user->nick.c_str());
        }
  ...
  public:
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        if (!allowcoreunload && InspIRCd::Match(parameters[0], "core_*.so", ascii_case_insensitive_map)) {
            user->WriteRemoteNumeric(ERR_CANTUNLOADMODULE, parameters[0],
        ...
        if (InspIRCd::Match(ServerInstance->Config->ServerName, servername)) {
            ...
            if (m) {
                if (ServerInstance->Modules->Unload(m)) {
                    ServerInstance->SNO->WriteToSnoMask('a',
                                                        "MODULE '%s' GLOBALLY UNLOADED BY '%s'",parameters[0].c_str(),
                                                        user->nick.c_str());
                    user->WriteRemoteNumeric(RPL_UNLOADEDMODULE, parameters[0],
            ...
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "MODULE '%s' GLOBAL UNLOAD BY '%s' (not unloaded here)",parameters[0].c_str(),
                                                user->nick.c_str());
        }
  ...
  public:
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        if (InspIRCd::Match(ServerInstance->Config->ServerName, servername)) {
            ...
            if (m) {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "MODULE '%s' GLOBALLY RELOADED BY '%s'", parameters[0].c_str(),
                                                    user->nick.c_str());
            ...
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "MODULE '%s' GLOBAL RELOAD BY '%s' (not reloaded here)",parameters[0].c_str(),
                                                user->nick.c_str());
        }
