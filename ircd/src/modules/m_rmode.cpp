  ...
  public:
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        if (mh == NULL || parameters[1].size() > 1) {
            user->WriteNumeric(ERR_UNKNOWNMODE, parameters[0],
                               "is not a recognised channel mode.");
            return CMD_FAILURE;
        }

        if (chan->GetPrefixValue(user) < mh->GetLevelRequired(false)) {
            user->WriteNumeric(Numerics::ChannelPrivilegesNeeded(chan,
                               mh->GetLevelRequired(false), InspIRCd::Format("unset channel mode %c (%s)",
        ...
        if ((pm = mh->IsPrefixMode())) {
            ...
            for (Channel::MemberMap::const_iterator it = users.begin(); it != users.end();
                    ++it) {
                if (!InspIRCd::Match(it->first->nick, pattern)) {
                    continue;
                }
                if (it->second->HasMode(pm) && !((it->first == user)
                                                 && (pm->GetPrefixRank() > VOICE_VALUE))) {
