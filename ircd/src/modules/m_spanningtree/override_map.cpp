...
static void GetDepthAndLen(TreeServer* current, unsigned int depth,
                           unsigned int& max_depth, unsigned int& max_len, unsigned int& max_version) {
    if (depth > max_depth) {
        max_depth = depth;
    }
    ...
    for (TreeServer::ChildServers::const_iterator i = servers.begin();
            i != servers.end(); ++i) {
        TreeServer* child = *i;
...
static std::vector<std::string> GetMap(User* user, TreeServer* current,
                                       unsigned int max_len, unsigned int max_version_len, unsigned int depth) {
    float percent = 0;
    ...
    if (user->IsOper()) {
        ...
        buffer += " [Up: " + InspIRCd::DurationString(secs_up) +
                  (current->rtt == 0 ? "]" : " Lag: " + ConvToStr(current->rtt) + "ms]");
    }
    ...
    for (TreeServer::ChildServers::const_iterator i = servers.begin();
        ...
        bool last = true;
        for (TreeServer::ChildServers::const_iterator j = i + 1; last
                && j != servers.end(); ++j)
            if (!IsHidden(user, *j)) {
                last = false;
            }
        ...
        std::vector<std::string> child_map = GetMap(user, child, next_len,
                                             max_version_len, depth + 1);

        for (std::vector<std::string>::const_iterator j = child_map.begin();
                j != child_map.end(); ++j) {
            const char* prefix;
...
CmdResult CommandMap::Handle(User* user, const Params& parameters) {
    ...
    std::vector<std::string> map = GetMap(user, Utils->TreeRoot, max, max_version,
                                          0);
    for (std::vector<std::string>::const_iterator i = map.begin(); i != map.end();
            ++i) {
    ...
    user->WriteRemoteNumeric(RPL_MAPUSERS,
                             InspIRCd::Format("%u server%s and %u user%s, average %.2f users per server",
                                     (unsigned int)Utils->serverlist.size(),
                                     (Utils->serverlist.size() > 1 ? "s" : ""), (unsigned int)totusers,
