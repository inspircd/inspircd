 ...
 */
bool SpanningTreeUtilities::DoCollision(User* u, TreeServer* server,
                                        time_t remotets, const std::string& remoteident, const std::string& remoteip,
    ...
    ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                              "Nick collision on \"%s\" caused by %s: %s/%lu/%s@%s %d <-> %s/%lu/%s@%s %d",
                              u->nick.c_str(), collidecmd,
                              u->uuid.c_str(), (unsigned long)localts, u->ident.c_str(),
                              u->GetIPString().c_str(), bChangeLocal,
                              remoteuid.c_str(), (unsigned long)remotets, remoteident.c_str(),
                              remoteip.c_str(), bChangeRemote);
    ...
    if (bChangeRemote) {
         ...
         */
        CmdBuilder("SAVE").push(remoteuid).push_int(remotets).Unicast(
            server->ServerUser);
    }
