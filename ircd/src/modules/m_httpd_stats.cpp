...
std::string Sanitize(const std::string& str) {
    ...
    for (std::string::const_iterator x = str.begin(); x != str.end(); ++x) {
        insp::flat_map<char, char const*>::const_iterator it = Entities::entities.find(
                    *x);
...
std::ostream& ServerInfo(std::ostream& data) {
    return data << "<server><name>" << ServerInstance->Config->ServerName <<
           "</name><description>"
...
std::ostream& ISupport(std::ostream& data) {
    data << "<isupport>";
    const std::vector<Numeric::Numeric>& isupport =
        ServerInstance->ISupport.GetLines();
    for (std::vector<Numeric::Numeric>::const_iterator i = isupport.begin();
            i != isupport.end(); ++i) {
        const Numeric::Numeric& num = *i;
        for (std::vector<std::string>::const_iterator j = num.GetParams().begin();
                j != num.GetParams().end() - 1; ++j) {
...
std::ostream& General(std::ostream& data) {
    data << "<general>";
    data << "<usercount>" << ServerInstance->Users->GetUsers().size() <<
         "</usercount>";
    data << "<localusercount>" << ServerInstance->Users->GetLocalUsers().size() <<
         "</localusercount>";
    data << "<channelcount>" << ServerInstance->GetChans().size() <<
         "</channelcount>";
    data << "<opercount>" << ServerInstance->Users->all_opers.size() <<
         "</opercount>";
    data << "<socketcount>" << (SocketEngine::GetUsedFds()) <<
         "</socketcount><socketmax>" << SocketEngine::GetMaxFds() << "</socketmax>";
    data << "<uptime><boot_time_t>" << ServerInstance->startup_time <<
         "</boot_time_t></uptime>";
...
std::ostream& XLines(std::ostream& data) {
    ...
    for (std::vector<std::string>::iterator it = xltypes.begin();
            it != xltypes.end(); ++it) {
...
std::ostream& Modules(std::ostream& data) {
    ...
    for (ModuleManager::ModuleMap::const_iterator i = mods.begin(); i != mods.end();
        ...
        data << "<module><name>" << i->first << "</name><description>" << Sanitize(
                 v.description) << "</description></module>";
    }
...
std::ostream& Channels(std::ostream& data) {
    ...
    for (chan_hash::const_iterator i = chans.begin(); i != chans.end(); ++i) {
        ...
        data << "<channel>";
        data << "<usercount>" << c->GetUsers().size() << "</usercount><channelname>" <<
             Sanitize(c->name) << "</channelname>";
        data << "<channeltopic>";
        ...
        for (Channel::MemberMap::const_iterator x = ulist.begin(); x != ulist.end();
                ++x) {
...
std::ostream& DumpUser(std::ostream& data, User* u) {
    data << "<user>";
    data << "<nickname>" << u->nick << "</nickname><uuid>" << u->uuid <<
         "</uuid><realhost>"
         << Sanitize(u->GetRealHost()) << "</realhost><displayhost>" << Sanitize(
             u->GetDisplayedHost()) << "</displayhost><realname>"
         << Sanitize(u->GetRealName()) << "</realname><server>" << u->server->GetName()
         << "</server><signon>"
         << u->signon << "</signon><age>" << u->age << "</age>";

    if (u->IsAway()) {
        data << "<away>" << Sanitize(u->awaymsg) << "</away><awaytime>" << u->awaytime
             << "</awaytime>";
    }
    ...
    data << "<modes>" << u->GetModeLetters().substr(1) << "</modes><ident>" <<
         Sanitize(u->ident) << "</ident>";
...
std::ostream& Servers(std::ostream& data) {
    ...
    for (ProtocolInterface::ServerList::const_iterator b = sl.begin();
            b != sl.end(); ++b) {
        data << "<server>";
...
std::ostream& Commands(std::ostream& data) {
    ...
    for (CommandParser::CommandMap::const_iterator i = commands.begin();
            i != commands.end(); ++i) {
        data << "<command><name>" << i->second->name << "</name><usecount>" <<
             i->second->use_count << "</usecount></command>";
    }
...
std::ostream& ListUsers(std::ostream& data, const HTTPQueryParameters& params) {
    ...
    size_t count = 0;
    for (NewUserList::const_iterator i = user_list.begin(); i != user_list.end()
            && (!limit || count < limit); ++i, ++count) {
        DumpUser(data, *i);
    }
...
class ModuleHttpStats : public Module, public HTTPRequestEventListener {
  ...
  public:
    ...
    ModResult HandleRequest(HTTPRequest* http) {
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "Handling HTTP request for %s",
                                  http->GetPath().c_str());
