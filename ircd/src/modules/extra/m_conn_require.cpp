...
class ModuleConnRequire : public Module {
    ...
    void SetZLine(User* user, time_t duration, const std::string& reason,
        ...
        const std::string expires = (duration == 0 ? "" :
                                     InspIRCd::Format(", expires in %s (on %s)",
        ...
        XLine* x = xlf->Generate(ServerInstance->Time(), duration, source, reason,
                                 mask);
        if (ServerInstance->XLines->AddLine(x, NULL)) {
            ServerInstance->SNO->WriteToSnoMask('x',
                                                "%s added %s Z-line for %s%s due to %s: %s", source.c_str(),
  ...
  public:
    ...
    void ReadConfig(ConfigStatus&) CXX11_OVERRIDE {
        ...
        for (ConfigIter i = tags.first; i != tags.second; ++i) {
            ...
            if (mask.empty()) {
                throw ModuleException("Missing \"mask\" in <badversion> tag at " +
                                      itag->getTagLocation());
                continue;
            }
    ...
    ModResult OnPreCommand(std::string& command, CommandBase::Params& parameters,
        ...
        if (!disableversion && ud->expectversion && !param.compare(1, ctcpversion.length(), ctcpversion)) {
            const std::string& rplversion = (param.length() > len_part ? param.substr(
                                                 len_part, param.length() - len_all) : "");
            ...
            for (std::vector<BadVersion>::const_iterator it = badversions.begin();
                    it != badversions.end(); ++it) {
                ...
                ServerInstance->SNO->WriteToSnoMask('u',
                                                    "Blocked user %s (%s) [%s] on port %d, version reply \"%s\" matched badversion mask \"%s\"",
                                                    user->GetFullRealHost().c_str(), user->GetIPString().c_str(),
                                                    user->GetRealName().c_str(), user->server_sa.port(), rplversion.c_str(),
                                                    bv.mask.c_str());
                ud->zapped = true;
            ...
            if (firstversionreply.empty()) {
                ...
                if (dualversion) {
                    ...
                    ClientProtocol::Messages::Privmsg ctcp(
            ...
            if (dualversion && (!firstversionreply.empty() && !secondversionreply.empty()
                                && firstversionreply != secondversionreply)) {
                if (dualban) {
                    SetZLine(user, dualduration, dualreason, "dual");
                }

                ServerInstance->SNO->WriteToSnoMask('u',
                                                    "Blocked user %s (%s) [%s] from connecting on port %d for mismatched version replies",
                                                    user->GetFullRealHost().c_str(), user->GetIPString().c_str(),
                                                    user->GetRealName().c_str(), user->server_sa.port());

                if (dualshow) {
                    ServerInstance->SNO->WriteToSnoMask('u', "Version replies \"%s\" and \"%s\"",
                                                        firstversionreply.c_str(), secondversionreply.c_str());
                }
    ...
    void OnUserDisconnect(LocalUser* user) CXX11_OVERRIDE {
        ...
        if (!ud->ccblocked) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Unregistered user exiting for unknown reasons: %s (%s) [%s]",
                                      user->GetFullRealHost().c_str(), user->GetIPString().c_str(),
