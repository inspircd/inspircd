  ...
  public:
    ...
    SQL::Field GetValue(int row, int column) {
        if ((row >= 0) && (row < rows) && (column >= 0)
                && (column < (int)fieldlists[row].size())) {
...
class SQLConnection : public SQL::Provider {
  private:
    bool EscapeString(SQL::Query* query, const std::string& in, std::string& out) {
        ...
        unsigned long escapedsize = mysql_escape_string(&buffer[0], in.c_str(),
                                    in.length());
        if (escapedsize == static_cast<unsigned long>(-1)) {
            SQL::Error err(SQL::QSEND_FAIL, InspIRCd::Format("%u: %s",
  ...
  public:
    ...
    bool Connect() {
        ...
        unsigned int ssl = config->getBool("ssl") ? SSL_MODE_REQUIRED :
                           SSL_MODE_PREFERRED;
        ...
        if (!mysql_real_connect(connection, host.c_str(), user.c_str(), pass.c_str(),
                                dbname.c_str(), port, NULL, CLIENT_IGNORE_SIGPIPE)) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Unable to connect to the %s MySQL server: %s",
        ...
        if (!charset.empty() && mysql_set_character_set(connection, charset.c_str())) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Could not set character set for %s to \"%s\": %s",
        ...
        if (!initialquery.empty()
                && mysql_real_query(connection, initialquery.data(), initialquery.length())) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Could not execute initial query \"%s\" for %s: %s",
    ...
    MySQLresult* DoBlockingQuery(const std::string& query) {
        ...
        if (CheckConnection()
                && !mysql_real_query(connection, query.data(), query.length())) {
            /* Successful query */
            ...
            SQL::Error e(SQL::QREPLY_FAIL, InspIRCd::Format("%u: %s",
                         mysql_errno(connection), mysql_error(connection)));
    ...
    void Submit(SQL::Query* call, const std::string& q,
                const SQL::ParamList& p) CXX11_OVERRIDE {
        std::string res;
    ...
    void Submit(SQL::Query* call, const std::string& q,
                const SQL::ParamMap& p) CXX11_OVERRIDE {
        std::string res;
        for(std::string::size_type i = 0; i < q.length(); i++) {
            if (q[i] != '$') {
                res.push_back(q[i]);
...
Version ModuleSQL::GetVersion() {
    return Version("Provides the ability for SQL modules to query a MySQL database.",
                   VF_VENDOR);
}
