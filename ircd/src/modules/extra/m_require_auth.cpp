...
class GALine : public XLine {
  ...
  public:
    GALine(time_t s_time, long d, const std::string& src, const std::string& re,
           const std::string& ident, const std::string& host, std::string othertext = "GA")
    ...
    void Apply(User* u) CXX11_OVERRIDE {
        if (!isLoggedIn(u)) {
            u->WriteNotice("*** NOTICE -- You need to identify via SASL to use this server (your host is "
                           + type + "-lined).");
...
class ALine : public GALine {
  public:
    ALine(time_t s_time, long d, const std::string& src, const std::string& re,
...
class ALineFactory : public XLineFactory {
  public:
    ...
    /** Generate an ALine
     */
    ALine* Generate(time_t set_time, unsigned long duration,
...
class GALineFactory : public XLineFactory {
  public:
    ...
    /** Generate a GALine
    */
    GALine* Generate(time_t set_time, unsigned long duration,
                     const std::string& source, const std::string& reason,
                     const std::string& xline_specific_mask) CXX11_OVERRIDE {
        IdentHostPair ih = ServerInstance->XLines->IdentSplit(xline_specific_mask);
  ...
  public:
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        if (parameters.size() >= 3) {
            ...
            XLine* al = xlf->Generate(ServerInstance->Time(), duration, user->nick,
                                      parameters[2], target);
            if (ServerInstance->XLines->AddLine(al, user)) {
                if (!duration) {
                    ServerInstance->SNO->WriteToSnoMask('x',
                                                        "%s added permanent %s-line for %s: %s", user->nick.c_str(), linename.c_str(),
                                                        target.c_str(), parameters[2].c_str());
                } else {
                    ServerInstance->SNO->WriteToSnoMask('x',
                                                        "%s added timed %s-line for %s, expires in %s (on %s): %s",
                                                        user->nick.c_str(), linename.c_str(), target.c_str(),
                ...
                delete al;
                user->WriteNotice("*** " + linename + "-line for " + target +
                                  " already exists.");
            }
        } else {
            std::string reason;
            if (ServerInstance->XLines->DelLine(target.c_str(), linename, reason, user)) {
                ...
                user->WriteNotice("*** " + linename + "-line " + target +
                                  " not found in list, try /stats " + ConvToStr(statschar) + ".");
            }
        }
