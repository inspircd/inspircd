...
void RemoveAll(const std::string& engine, ChanModeReference& ban,
               ChanModeReference& exc, ChanModeReference& inv) {
    ...
    for (chan_hash::const_iterator c = chans.begin(); c != chans.end(); ++c) {
        ...
        for (std::vector<ListModeBase*>::const_iterator i = listmodes.begin();
                i != listmodes.end(); ++i) {
            ListModeBase* lm = *i;
            ...
            for (ListModeBase::ModeList::const_iterator iter = list->begin();
                    iter != list->end(); ++iter) {
        ...
        const std::string msg = "Regex engine has changed to '" + engine +
                                "'. All regex extbans have been removed";
        ...
        ClientProtocol::Messages::Privmsg notice(ServerInstance->FakeClient, chan, msg,
                MSG_NOTICE);
...
class WatchedMode : public ModeWatcher {
  ...
  public:
    WatchedMode(Module *mod, bool& oo, dynamic_reference<RegexFactory>& rf,
    ...
    bool BeforeMode(User* user, User*, Channel* chan, std::string& param,
        ...
        if (!rxfactory) {
            user->WriteNumeric(ERR_NOENGINE,
                               "Regex engine is missing, cannot set a regex extban.");
            return false;
        }
        ...
        if (plink == std::string::npos || mask.find('@', plink) == std::string::npos) {
            user->WriteNumeric(ERR_INVALIDMASK, mask,
                               "Regex extban mask must be n!u@h\\sr");
            return false;
        }
        ...
        try {
            ...
            user->WriteNumeric(ERR_INVALIDMASK, mask,
                               InspIRCd::Format("Regex extban mask is invalid (%s)",
                                                e.GetReason().c_str()));
            return false;
        }
        ...
        if (!rxfactory) {
            if (newrxengine.empty()) {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "WARNING: No regex engine loaded - regex extban functionality disabled until this is corrected.");
            } else {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "WARNING: Regex engine '%s' is not loaded - regex extban functionality disabled until this is corrected.",
                                                    newrxengine.c_str());
            }
            ...
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "Regex engine has changed to '%s', removing all regex extbans.",
                                                rxfactory->name.c_str());
    ...
    ModResult OnCheckBan(User* user, Channel* chan,
        ...
        if (timediff > 0.5) {
            ServerInstance->SNO->WriteGlobalSno('a',
                                                "*** extbanregex match took %f seconds on %s %s",
