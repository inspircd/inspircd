...
class ProviderConfig {
  private:
    static Argon2_version SanitizeArgon2Version(unsigned long version) {
        ...
        ServerInstance->Logs->Log("MODULE", LOG_DEFAULT,
                                  "Unknown Argon2 version (%lu) specified; assuming 13",
                                  version);
        return ARGON2_VERSION_13;
    }
  ...
  public:
    ...
    ProviderConfig(const std::string& tagname, ProviderConfig* def) {
        ...
        this->lanes = tag->getUInt("lanes", def_lanes, ARGON2_MIN_LANES,
                                   ARGON2_MAX_LANES);

        uint32_t def_memory = def ? def->memory : 131072; // 128 MiB
        this->memory = tag->getUInt("memory", def_memory, ARGON2_MIN_MEMORY,
                                    ARGON2_MAX_MEMORY);

        uint32_t def_outlen = def ? def->outlen : 32;
        this->outlen = tag->getUInt("length", def_outlen, ARGON2_MIN_OUTLEN,
                                    ARGON2_MAX_OUTLEN);

        uint32_t def_saltlen = def ? def->saltlen : 16;
        this->saltlen = tag->getUInt("saltlength", def_saltlen, ARGON2_MIN_SALT_LENGTH,
                                     ARGON2_MAX_SALT_LENGTH);

        uint32_t def_threads = def ? def->threads : 1;
        this->threads = tag->getUInt("threads", def_threads, ARGON2_MIN_THREADS,
                                     ARGON2_MAX_THREADS);
  ...
  public:
    ...
    std::string GenerateRaw(const std::string& data) CXX11_OVERRIDE {
        ...
        if (argonResult != ARGON2_OK) {
            throw ModuleException("Argon2 hashing failed!: " + std::string(
                                      argon2_error_message(argonResult)));
        }
