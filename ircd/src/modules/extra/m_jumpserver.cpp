...
class CommandJumpserver : public Command {
  public:
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        if (parameters.empty()) {
            if (port) {
                user->WriteNotice("*** Disabled jumpserver (previously set to '" + redirect_to +
                                  ":" + ConvToStr(port) + "')");
            } else {
        ...
        if (parameters.size() >= 3) {
            for (std::string::const_iterator n = parameters[2].begin();
                    n != parameters[2].end(); ++n) {
                switch (*n) {
                case '+':
                    direction = true;
                    break;
            ...
            port = ConvToNum<int>(parameters[1].substr(0,
                                  delimpos ? delimpos : std::string::npos));
            sslport = (delimpos == std::string::npos ? 0 : ConvToNum<int>
                       (parameters[1].substr(delimpos + 1)));
            ...
            if (redirect_all_immediately) {
                ...
                for (UserManager::LocalList::const_iterator i = list.begin(); i != list.end();
                    ...
                    ++i;
                    if (!t->IsOper()) {
                        t->WriteNumeric(RPL_REDIR, parameters[0], GetPort(t),
                                        "Please use this Server/Port instead");
            ...
            user->WriteNotice("*** Set jumpserver to server '" + parameters[0] + "' port '"
                              + (port ? ConvToStr(port) : "Auto") + ", SSL " + (sslport ? ConvToStr(
                                          sslport) : "Auto") + "', flags '+" +
  ...
  public:
    ...
    ModResult OnUserRegister(LocalUser* user) CXX11_OVERRIDE {
        if (js.redirect_new_users) {
            ...
            user->WriteNumeric(RPL_REDIR, js.redirect_to, port,
                               "Please use this Server/Port instead");
