...
bool CanCheck(Channel* chan, User* user, ChanModeReference& ban) {
    ...
    if (ban->GetLevelRequired(true) > chan->GetPrefixValue(user)) {
        user->WriteNumeric(ERR_CHANOPRIVSNEEDED, chan->name,
                           "You do not have access to modify the ban list.");
        return false;
    }
...
void CheckLists(User* source, Channel* chan, User* user, ChanModeReference& ban,
    ...
    if (list) {
        for (iter = list->begin(); iter != list->end(); ++iter) {
            ...
            source->WriteNumeric(RPL_BANMATCH, chan->name,
                                 InspIRCd::Format("Ban %s matches %s (set by %s on %s)",
    ...
    if (list) {
        for (iter = list->begin(); iter != list->end(); ++iter) {
            ...
            source->WriteNumeric(RPL_EXCEPTIONMATCH, chan->name,
                                 InspIRCd::Format("Exception %s matches %s (set by %s on %s)",
                                                  iter->mask.c_str(), user->nick.c_str(), iter->setter.c_str(),
                                                  ServerInstance->TimeString(iter->time, "%Y-%m-%d %H:%M:%S UTC", true).c_str()));
        }
    }
...
class CommandCheckBans : public Command {
  ...
  public:
    CommandCheckBans(Module* Creator, ChanModeReference& _ban,
                     ChanModeReference& _exc)
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        for (Channel::MemberMap::const_iterator u = users.begin(); u != users.end(); ++u) {
            if (chan->CheckBan(u->first, parameters[1])) {
                user->WriteNumeric(RPL_BANMATCH, chan->name,
                                   InspIRCd::Format("Mask %s matches %s",
        ...
        if (matched > 0) {
            ...
            user->WriteNumeric(RPL_BANMATCH, chan->name,
                               InspIRCd::Format("Mask %s matched %d of %lu users (%.2f%%).",
