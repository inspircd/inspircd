...
class TOTP {
  public:
    ...
    std::string Generate(const std::string secret, unsigned long time = 0) {
        ...
        std::string hash = Hash->hmac(key, std::string(challenge.begin(),
                                      challenge.end()));
...
class CommandTOTP : public Command {
    ...
    void ShowCode(User* user, const std::string& secret,
                  const std::string& label = "") {
        std::string url =
            "https://www.google.com/chart?chs=200x200&chld=M|0&cht=qr&chl=otpauth%3A%2F%2Ftotp%2F"
            + ServerInstance->Config->Network + (!label.empty() ? "%20(" + label + ")" : "")
            + "%3Falgorithm%3D"
  ...
  public:
    ...
    ModResult OnPreCommand(std::string& command, CommandBase::Params& parameters,
        ...
        if (validated && command == "OPER" && parameters.size() > 1) {
            ServerConfig::OperIndex::iterator it = ServerInstance->Config->oper_blocks.find(
                parameters[0]);
