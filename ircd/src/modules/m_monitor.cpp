...
class IRCv3::Monitor::Manager {
    ...
    class ExtItem : public ExtensionItem {
      ...
      public:
        ...
        std::string ToInternal(const Extensible* container,
            ...
            for (WatchedList::const_iterator i = extdata->list.begin();
                    i != extdata->list.end(); ++i) {
                const Entry* entry = *i;
  ...
  public:
    ...
    WatchResult Watch(LocalUser* user, const std::string& nick,
  ...
  private:
    typedef TR1NS::unordered_map<std::string, Entry, irc::insensitive, irc::StrHashComp>
    NickHash;
    ...
    Entry* AddWatcher(const std::string& nick, LocalUser* user) {
        std::pair<NickHash::iterator, bool> ret = nicks.insert(std::make_pair(nick,
                Entry()));
    ...
    bool RemoveWatcher(const std::string& nick, LocalUser* user,
...
void IRCv3::Monitor::Manager::ExtItem::FromInternal(Extensible* container,
...
class CommandMonitor : public SplitCommand {
    ...
    void HandlePlus(LocalUser* user, const std::string& input) {
        ...
        for (std::string nick; ss.GetToken(nick); ) {
            IRCv3::Monitor::Manager::WatchResult result = manager.Watch(user, nick,
                    maxmonitor);
            if (result == IRCv3::Monitor::Manager::WR_TOOMANY) {
                ...
                user->WriteNumeric(ERR_MONLISTFULL, maxmonitor, InspIRCd::Format("%s%s%s",
                                   nick.c_str(), (ss.StreamEnd() ? "" : ","), ss.GetRemaining().c_str()),
            ...
            ReplyBuilder& out = (IRCv3::Monitor::Manager::FindNick(nick) ? online :
                                 offline);
            out.Add(nick);
        }
  ...
  public:
    ...
    CmdResult HandleLocal(LocalUser* user,
        ...
        if (subcmd == '+') {
            ...
            for (IRCv3::Monitor::WatchedList::const_iterator i = list.begin();
                    i != list.end(); ++i) {
            ...
            for (IRCv3::Monitor::WatchedList::const_iterator i = list.begin();
                    i != list.end(); ++i) {
                IRCv3::Monitor::Entry* entry = *i;
                ReplyBuilder& out = (IRCv3::Monitor::Manager::FindNick(entry->GetNick()) ?
                                     online : offline);
                out.Add(entry->GetNick());
            }
...
class ModuleMonitor
    ...
    void SendAlert(unsigned int numeric, const std::string& nick) {
        ...
        for (IRCv3::Monitor::WatcherList::const_iterator i = list->begin();
                i != list->end(); ++i) {
            LocalUser* curr = *i;
  ...
  public:
    ...
    void ForEachWatcher(User* user, Monitor::ForEachHandler& handler,
                        bool extended_only) CXX11_OVERRIDE {
