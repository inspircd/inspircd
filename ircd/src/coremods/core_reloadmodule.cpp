...
class DataKeeper {
    ...
    struct ModesExts {
        ...
        bool empty() const {
            return ((modelist.empty()) && (extlist.empty()));
        }
    ...
    void SaveExtensions(Extensible* extensible,
                        std::vector<InstanceData>& extdatalist);
    void SaveMemberData(Channel* chan,
                        std::vector<ChanData::MemberData>& memberdatalist);
    static void SaveListModes(Channel* chan, ListModeBase* lm, size_t index,
                              ModesExts& currdata);
     ...
     */
    void RestoreObj(const OwnedModesExts& data, Extensible* extensible,
                    ModeType modetype, Modes::ChangeList& modechange);
     ...
     */
    void RestoreExtensions(const std::vector<InstanceData>& list,
     ...
     * @param modechange Mode change to populate with the modes
     */
    void RestoreModes(const std::vector<InstanceData>& list, ModeType modetype,
     ...
     */
    void RestoreMemberData(Channel* chan,
                           const std::vector<ChanData::MemberData>& memberdatalist,
                           Modes::ChangeList& modechange);
...
void DataKeeper::SaveExtensions(Extensible* extensible,
    ...
    size_t index = 0;
    for (std::vector<ProviderInfo>::const_iterator i = handledexts.begin();
            i != handledexts.end(); ++i, index++) {
...
void DataKeeper::SaveListModes(Channel* chan, ListModeBase* lm, size_t index,
                               ModesExts& currdata) {
...
void DataKeeper::SaveMemberData(Channel* chan,
                                std::vector<OwnedModesExts>& memberdatalist) {
    ModesExts currdata;
    ...
    for (Channel::MemberMap::const_iterator i = users.begin(); i != users.end();
        ...
        for (size_t j = 0; j < handledmodes[MODETYPE_CHANNEL].size(); j++) {
            ...
            if ((pm) && (memb->HasMode(pm))) {
                currdata.modelist.push_back(InstanceData(j,
                                            memb->user->uuid));    // Need to pass the user's uuid to the mode parser to set the mode later
            }
        }
...
void DataKeeper::RestoreMemberData(Channel* chan,
                                   const std::vector<ChanData::MemberData>& memberdatalist,
                                   Modes::ChangeList& modechange) {
    for (std::vector<ChanData::MemberData>::const_iterator i =
                memberdatalist.begin(); i != memberdatalist.end(); ++i) {
        const ChanData::MemberData& md = *i;
        User* const user = ServerInstance->FindUUID(md.owner);
        if (!user) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "User %s is gone (while processing %s)", md.owner.c_str(), chan->name.c_str());
            continue;
        }
        ...
        if (!memb) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Member %s is no longer on channel %s", md.owner.c_str(), chan->name.c_str());
            continue;
        }
...
void DataKeeper::Save(Module* currmod) {
    ...
    for (ExtensionManager::ExtMap::const_iterator i = allexts.begin();
            i != allexts.end(); ++i) {
    ...
    FOREACH_MOD_CUSTOM(*reloadevprov, ReloadModule::EventListener,
                       OnReloadModuleSave, (mod, this->moddata));

    ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                              "Saved data about %lu users %lu chans %lu modules",
                              (unsigned long)userdatalist.size(), (unsigned long)chandatalist.size(),
                              (unsigned long)moddata.list.size());
}

void DataKeeper::VerifyServiceProvider(const ProviderInfo& service,
                                       const char* type) {
    const ServiceProvider* sp = service.extitem;
    if (!sp) {
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "%s \"%s\" is no longer available", type, service.itemname.c_str());
    } else if (sp->creator != mod) {
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "%s \"%s\" is now handled by %s",
                                  type, service.itemname.c_str(),
                                  (sp->creator ? sp->creator->ModuleSourceFile.c_str() : "<core>"));
    }
}

void DataKeeper::LinkModes(ModeType modetype) {
    std::vector<ProviderInfo>& list = handledmodes[modetype];
    for (std::vector<ProviderInfo>::iterator i = list.begin(); i != list.end();
            ++i) {
        ProviderInfo& item = *i;
        item.mh = ServerInstance->Modes->FindMode(item.itemname, modetype);
        VerifyServiceProvider(item,
                              (modetype == MODETYPE_USER ? "User mode" : "Channel mode"));
    }
}

void DataKeeper::LinkExtensions() {
    for (std::vector<ProviderInfo>::iterator i = handledexts.begin();
            i != handledexts.end(); ++i) {
        ProviderInfo& item = *i;
...
void DataKeeper::LinkSerializers() {
    for (std::vector<ProviderInfo>::iterator i = handledserializers.begin();
            i != handledserializers.end(); ++i) {
        ProviderInfo& item = *i;
        item.serializer =
            ServerInstance->Modules.FindDataService<ClientProtocol::Serializer>
            (item.itemname);
...
void DataKeeper::Fail() {
    ...
    ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                              "Restore failed, notifying modules");
    DoRestoreModules();
}

void DataKeeper::RestoreObj(const OwnedModesExts& data, Extensible* extensible,
                            ModeType modetype, Modes::ChangeList& modechange) {
    RestoreExtensions(data.extlist, extensible);
    RestoreModes(data.modelist, modetype, modechange);
}

void DataKeeper::RestoreExtensions(const std::vector<InstanceData>& list,
                                   Extensible* extensible) {
    for (std::vector<InstanceData>::const_iterator i = list.begin();
            i != list.end(); ++i) {
...
void DataKeeper::RestoreModes(const std::vector<InstanceData>& list,
                              ModeType modetype, Modes::ChangeList& modechange) {
    for (std::vector<InstanceData>::const_iterator i = list.begin();
            i != list.end(); ++i) {
        const InstanceData& id = *i;
...
void DataKeeper::DoRestoreUsers() {
    ...
    for (std::vector<UserData>::const_iterator i = userdatalist.begin();
            i != userdatalist.end(); ++i) {
        const UserData& userdata = *i;
        User* const user = ServerInstance->FindUUID(userdata.owner);
        if (!user) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "User %s is gone",
                                      userdata.owner.c_str());
            continue;
        }
        ...
        ServerInstance->Modes.Process(ServerInstance->FakeClient, NULL, user,
                                      modechange, ModeParser::MODE_LOCALONLY);
        modechange.clear();
    }
}
...
void DataKeeper::DoRestoreChans() {
    ...
    for (std::vector<ChanData>::const_iterator i = chandatalist.begin();
            i != chandatalist.end(); ++i) {
        const ChanData& chandata = *i;
        Channel* const chan = ServerInstance->FindChan(chandata.owner);
        if (!chan) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "Channel %s not found",
                                      chandata.owner.c_str());
            continue;
        }
...
void DataKeeper::DoRestoreModules() {
    for (ReloadModule::CustomData::List::iterator i = moddata.list.begin();
            i != moddata.list.end(); ++i) {
        ReloadModule::CustomData::Data& data = *i;
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "Calling module data handler %p",
                                  (void*)data.handler);
...
class ReloadAction : public ActionBase {
  ...
  public:
    ReloadAction(Module* m, const std::string& uid,
                 const std::string& passedmodname)
        : mod(m)
        , uuid(uid)
    ...
    void Call() CXX11_OVERRIDE {
        ...
        if (result) {
            ...
            ServerInstance->SNO->WriteGlobalSno('a', "The %s module was reloaded.",
                                                passedname.c_str());
        } else {
            datakeeper.Fail();
            ServerInstance->SNO->WriteGlobalSno('a', "Failed to reload the %s module.",
                                                passedname.c_str());
        }
        ...
        if (user) {
            if (result) {
                user->WriteNumeric(RPL_LOADEDMODULE, passedname,
                                   InspIRCd::Format("The %s module was reloaded.", passedname.c_str()));
            } else {
                user->WriteNumeric(ERR_CANTUNLOADMODULE, passedname,
                                   InspIRCd::Format("Failed to reload the %s module.", passedname.c_str()));
            }
        }
...
CmdResult CommandReloadmodule::Handle(User* user, const Params& parameters) {
    ...
    if (m == creator) {
        user->WriteNumeric(ERR_CANTUNLOADMODULE, parameters[0],
                           "You cannot reload core_reloadmodule (unload and load it)");
        return CMD_FAILURE;
    }
    ...
    if ((m) && (ServerInstance->Modules.CanUnload(m))) {
        ServerInstance->AtomicActions.AddAction(new ReloadAction(m, user->uuid,
                                                parameters[0]));
        return CMD_SUCCESS;
    } else {
        user->WriteNumeric(ERR_CANTUNLOADMODULE, parameters[0],
                           "Could not find a loaded module by that name");
        return CMD_FAILURE;
    }
}
