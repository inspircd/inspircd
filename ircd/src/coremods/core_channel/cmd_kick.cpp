...
CmdResult CommandKick::Handle(User* user, const Params& parameters) {
    ...
    if (IS_LOCAL(user)) {
        ...
        if (!srcmemb) {
            user->WriteNumeric(ERR_NOTONCHANNEL, parameters[0],
                               "You're not on that channel!");
            return CMD_FAILURE;
        }

        if (u->server->IsULine()) {
            user->WriteNumeric(ERR_RESTRICTED, c->name,
                               "You may not kick a U-lined client");
            return CMD_FAILURE;
        }
    }
    ...
    if (victimiter == c->userlist.end()) {
        user->WriteNumeric(ERR_USERNOTINCHANNEL, u->nick, c->name,
    ...
    if ((!IS_LOCAL(user)) && (parameters.size() > 3)) {
        ...
        if (memb->id != Membership::IdFromString(parameters[2])) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Dropped KICK due to membership id mismatch: " + ConvToStr(
    ...
    const std::string reason((has_reason ? parameters.back() : user->nick), 0,
                             ServerInstance->Config->Limits.MaxKick);
    ...
    if (srcmemb) {
        ...
        if (res == MOD_RES_PASSTHRU) {
            ...
            for (std::string::size_type i = 0; i < memb->modes.length(); i++) {
                ModeHandler* mh = ServerInstance->Modes->FindMode(memb->modes[i],
                                  MODETYPE_CHANNEL);
            ...
            if (them < req) {
                user->WriteNumeric(Numerics::ChannelPrivilegesNeeded(memb->chan, req,
                                   "kick a more privileged user"));
                return CMD_FAILURE;
            }
        }
    }
