...
class Manager {
  public:
     ...
     */
    void UpdateConfig(unsigned int NewGroupSize, unsigned int NewMaxGroups,
  ...
  private:
    ...
    typedef TR1NS::unordered_map<std::string, WhoWas::Nick*, irc::insensitive, irc::StrHashComp>
    whowas_users;
...
CmdResult CommandWhowas::Handle(User* user, const Params& parameters) {
    ...
    if (!nick) {
        user->WriteNumeric(ERR_WASNOSUCHNICK, parameters[0],
        ...
        for (WhoWas::Nick::List::const_reverse_iterator i = list.rbegin(); i != last;
                ++i) {
            WhoWas::Entry* u = *i;

            user->WriteNumeric(RPL_WHOWASUSER, parameters[0], u->ident, u->dhost, '*',
                               u->real);

            if (user->HasPrivPermission("users/auspex")) {
                user->WriteNumeric(RPL_WHOWASIP, parameters[0],
                                   InspIRCd::Format("was connecting from *@%s", u->host.c_str()));
            }

            std::string signon = InspIRCd::TimeString(u->signon);
            bool hide_server = (!ServerInstance->Config->HideServer.empty()
                                && !user->HasPrivPermission("servers/auspex"));
            user->WriteNumeric(RPL_WHOISSERVER, parameters[0],
                               (hide_server ? ServerInstance->Config->HideServer : u->server), signon);
        }
    }
...
const WhoWas::Nick* WhoWas::Manager::FindNick(const std::string& nickname)
const {
...
void WhoWas::Manager::Add(User* user) {
    ...
    std::pair<whowas_users::iterator, bool> ret = whowas.insert(std::make_pair(
                user->nick, static_cast<WhoWas::Nick*>(NULL)));
...
void WhoWas::Manager::UpdateConfig(unsigned int NewGroupSize,
                                   unsigned int NewMaxGroups, unsigned int NewMaxKeep) {
    if ((NewGroupSize == GroupSize) && (NewMaxGroups == MaxGroups)
            && (NewMaxKeep == MaxKeep)) {
        return;
    }
...
void WhoWas::Manager::PurgeNick(WhoWas::Nick* nick) {
    ...
    if (it == whowas.end()) {
        ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                  "ERROR: Inconsistency detected in whowas database, please report");
        return;
    }
    PurgeNick(it);
}
  ...
  public:
    ...
    ModResult OnStats(Stats::Context& stats) CXX11_OVERRIDE {
        if (stats.GetSymbol() == 'z') {
            stats.AddRow(249, "Whowas entries: "+ConvToStr(
                             cmd.manager.GetStats().entrycount));
        }
