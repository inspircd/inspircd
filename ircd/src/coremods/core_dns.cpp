...
class Packet : public Query {
    void PackName(unsigned char* output, unsigned short output_size,
                  unsigned short& pos, const std::string& name) {
    ...
    std::string UnpackName(const unsigned char* input, unsigned short input_size,
                           unsigned short& pos) {
        std::string name;
    ...
    ResourceRecord UnpackResourceRecord(const unsigned char* input,
        ...
        case QUERY_A: {
            ...
            addrs.in4.sin_addr.s_addr = input[pos] | (input[pos + 1] << 8) |
        ...
        case QUERY_TXT: {
            ...
            record.rdata = std::string(reinterpret_cast<const char *>(input + pos),
                                       rdlength);
            pos += rdlength;
  ...
  public:
    ...
    void Fill(const unsigned char* input, const unsigned short len) {
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "qdcount: " + ConvToStr(qdcount) + " ancount: " + ConvToStr(
                                      ancount) + " nscount: " + ConvToStr(nscount) + " arcount: " + ConvToStr(
                                      arcount));
...

        {
            ...
            if (q.type == QUERY_PTR) {
                ...
                if (q.name.find(':') != std::string::npos) {
                    ...
                    ip.in4.sin_addr.s_addr = forward << 24 | (forward & 0xFF00) << 8 |
                                             (forward & 0xFF0000) >> 8 | forward >> 24;
...
class MyManager : public Manager, public Timer, public EventHandler {
    ...
    bool CheckCache(DNS::Request* req, const DNS::Question& question) {
        ServerInstance->Logs->Log(MODNAME, LOG_SPARSE,
                                  "cache: Checking cache for " + question.name);
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "cache: Using cached result for " + question.name);
        record.cached = true;
    ...
    void AddCache(Query& r) {
        ...
        for (std::vector<ResourceRecord>::const_iterator i = r.answers.begin();
                i != r.answers.end(); ++i) {
        ...
        rr.ttl = cachettl;
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "cache: added cache for " + rr.name + " -> " + rr.rdata + " ttl: " + ConvToStr(
                                      rr.ttl));
  ...
  public:
    ...
    void OnEventHandlerRead() CXX11_OVERRIDE {
        ...
        if (myserver != from) {
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Got a result from the wrong server! Bad NAT or DNS forging attempt? '%s' != '%s'",
        ...
        if (request->question != recv_packet.question) {
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Received an answer that isn't for a question we asked");
            return;
        }
        ...
        if (!valid) {
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Lookup complete for " + request->question.name);
    ...
    bool Tick(time_t now) CXX11_OVERRIDE {
        ...
        if (expired) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "cache: purged %lu expired DNS entries", expired);
        }

        return true;
    }

    void Rehash(const std::string& dnsserver, std::string sourceaddr,
        ...
        if (this->HasFd()) {
            ...
            if (SocketEngine::Bind(this->GetFd(), bindto) < 0) {
                /* Failed to bind */
                ServerInstance->Logs->Log(MODNAME, LOG_SPARSE,
                                          "Error binding dns socket - hostnames will NOT resolve");
                SocketEngine::Close(this->GetFd());
                this->SetFd(-1);
            } else if (!SocketEngine::AddFd(this, FD_WANT_POLL_READ | FD_WANT_NO_WRITE)) {
                ServerInstance->Logs->Log(MODNAME, LOG_SPARSE,
                                          "Internal error starting DNS - hostnames will NOT resolve.");
                SocketEngine::Close(this->GetFd());
                this->SetFd(-1);
            }
            ...
            if (bindto.family() != myserver.family()) {
                ServerInstance->Logs->Log(MODNAME, LOG_SPARSE,
                                          "Nameserver address family differs from source address family - hostnames might not resolve");
            }
        } else {
            ServerInstance->Logs->Log(MODNAME, LOG_SPARSE,
                                      "Error creating DNS socket - hostnames will NOT resolve");
        }
    }
};
...
class ModuleDNS : public Module {
    ...
    void FindDNSServer() {
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                  "WARNING: <dns:server> not defined, attempting to find a working server in the system settings...");
        ...
        if (pFixedInfo) {
            ...
            if (!DNSServer.empty()) {
                ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                          "<dns:server> set to '%s' as first active resolver in the system settings.",
                                          DNSServer.c_str());
                return;
            }
        }
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                  "No viable nameserver found! Defaulting to nameserver '127.0.0.1'!");
#else
        // attempt to look up their nameserver from /etc/resolv.conf
        ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                  "WARNING: <dns:server> not defined, attempting to find working server in /etc/resolv.conf...");
        ...
        while (resolv >> DNSServer) {
            if (DNSServer == "nameserver") {
                ...
                if (DNSServer.find_first_not_of("0123456789.") == std::string::npos
                        || DNSServer.find_first_not_of("0123456789ABCDEFabcdef:") ==
                        std::string::npos) {
                    ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                              "<dns:server> set to '%s' as first resolver in /etc/resolv.conf.",
                                              DNSServer.c_str());
                    return;
                }
            }
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                  "/etc/resolv.conf contains no viable nameserver entries! Defaulting to nameserver '127.0.0.1'!");
#endif
        DNSServer = "127.0.0.1";
    }
