...
void SocketEngine::InitError() {
    std::cerr << con_red << "FATAL ERROR!" << con_reset <<
              " Socket engine initialization failed. " << strerror(errno) << '.' << std::endl;
...
int SocketEngine::RecvFrom(EventHandler* fd, void *buf, size_t len, int flags,
...
int SocketEngine::Send(EventHandler* fd, const void *buf, size_t len,
                       int flags) {
...
int SocketEngine::SendTo(EventHandler* fd, const void* buf, size_t len,
                         int flags, const irc::sockets::sockaddrs& address) {
    int nbSent = sendto(fd->GetFd(), (const char*)buf, len, flags, &address.sa,
                        address.sa_size());
    stats.UpdateWriteCounters(nbSent);
    return nbSent;
}
...
void SocketEngine::Statistics::GetBandwidth(float& kbitpersec_in,
        float& kbitpersec_out, float& kbitpersec_total) const {
    CheckFlush();
