...
void XLineManager::CheckELines() {
    ...
    for (UserManager::LocalList::const_iterator u2 = list.begin(); u2 != list.end();
            u2++) {
        LocalUser* u = *u2;
        u->exempt = false;
...
bool XLineManager::AddLine(XLine* line, User* user) {
    ...
    ServerInstance->BanCache.RemoveEntries(line->type,
                                           false); // XXX perhaps remove ELines here?
...
bool XLineManager::DelLine(const char* hostmask, const std::string& type,
                           std::string& reason, User* user, bool simulate) {
...
void XLineManager::ApplyLines() {
    ...
    for (UserManager::LocalList::const_iterator j = list.begin(); j != list.end();
        ...
        for (std::vector<XLine *>::iterator i = pending_lines.begin();
                i != pending_lines.end(); i++) {
            XLine *x = *i;
            if (x->Matches(u)) {
                x->Apply(u);
...
void XLineManager::InvokeStats(const std::string& type, unsigned int numeric,
                               Stats::Context& stats) {
    ContainerIter n = lookup_lines.find(type);
    ...
    if (n != lookup_lines.end()) {
        ...
        for (LookupIter i = list.begin(); i != list.end(); ) {
            ...
            if (e->duration && current > e->expiry) {
                ...
                stats.AddRow(numeric, e->Displayable(), e->set_time, e->duration, e->source,
                             e->reason);
            }
            i = safei;
        }
...
bool XLineManager::InvokeStats(const std::string& type,
    ...
    for (LookupIter liter = citer->second.begin(); liter != citer->second.end(); ) {
        ...
        context.AddRow(RPL_STATS, context.GetSymbol(), xline->Displayable(),
                       xline->set_time, xline->duration, xline->source, xline->reason);
    }
    return true;
}
...
XLineManager::~XLineManager() {
    ...
    for (XLineContainer::iterator i = lookup_lines.begin(); i != lookup_lines.end();
            i++) {
...
void XLine::DefaultApply(User* u, const std::string &line, bool bancache) {
    ...
    if (bancache) {
        ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                                  "BanCache: Adding positive hit (" + line + ") for " + u->GetIPString());
        ServerInstance->BanCache.AddHit(u->GetIPString(), this->type, banReason,
                                        (this->duration > 0 ? (this->expiry - ServerInstance->Time()) : 0));
    }
}
...
bool KLine::Matches(User *u) {
    ...
    if (InspIRCd::Match(u->ident, this->identmask, ascii_case_insensitive_map)) {
        if (InspIRCd::MatchCIDR(u->GetRealHost(), this->hostmask,
...
void XLine::DisplayExpiry() {
    ...
    ServerInstance->SNO->WriteToSnoMask('x',
                                        "Removing an expired %s%s on %s (set by %s %s ago): %s",
                                        type.c_str(), (onechar ? "-line" : ""), Displayable().c_str(), source.c_str(),
                                        InspIRCd::DurationString(ServerInstance->Time() - set_time).c_str(),
                                        reason.c_str());
}
