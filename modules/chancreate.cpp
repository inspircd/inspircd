 ...
 *
 *   Copyright (C) 2019-2021, 2025 Sadie Powell <sadie@sadiepowell.dev>
...
public:
	ModuleChanCreate()
		: Module(VENDOR, "Sends a notice to snomasks j (local) and J (remote) when a channel is created.")
	{
	}
	...
	void OnUserJoin(Membership* memb, bool sync, bool created, User::List& except) override
	{
		if ((created) && (memb->user->IsLocal()))
		{
			ServerInstance->SNO.WriteGlobalSno('j', "Channel {} created by {}",
				memb->chan->name, memb->user->GetDescription());
		}
	}
};
