 ...
 *
 *   Copyright (C) 2017-2021, 2023, 2025 Sadie Powell <sadie@sadiepowell.dev>
...
public:
	...
	ModResult OnUserPreNick(LocalUser* user, const std::string& newnick) override
	{
		for (const auto* memb : user->chans)
		{
			...
			{
				user->WriteNumeric(ERR_CANTCHANGENICK, FMT::format("Can't change nickname while on {} ({})", memb->chan->name,
					modeset ? FMT::format("+{} is set", nn.GetModeChar()) : "you're extbanned"));
				return MOD_RES_DENY;
			}
		}
