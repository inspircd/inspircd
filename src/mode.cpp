...
ModeHandler::ModeHandler(Module* Creator, const std::string& Name,
                         char modeletter, ParamSpec Params, ModeType type, Class mclass)
...
ModeAction ModeHandler::OnModeChange(User*, User*, Channel*, std::string&,
                                     bool) {
...
void ModeHandler::OnParameterMissing(User* user, User* dest, Channel* channel) {
    std::string message =
        InspIRCd::Format("You must specify a parameter for the %s mode.", name.c_str());
    if (!syntax.empty()) {
        message.append(InspIRCd::Format(" Syntax: %s.", syntax.c_str()));
    }

    if (channel) {
        user->WriteNumeric(Numerics::InvalidModeParameter(channel, this, "*",
                           message));
    } else {
...
void ModeHandler::OnParameterInvalid(User* user, Channel* targetchannel,
                                     User* targetuser, const std::string& parameter) {
    if (targetchannel) {
...
bool ModeHandler::ResolveModeConflict(std::string& theirs,
                                      const std::string& ours, Channel*) {
    return (theirs < ours);
}

void ModeHandler::RegisterService() {
    ServerInstance->Modes.AddMode(this);
    ServerInstance->Modules.AddReferent((GetModeType() == MODETYPE_CHANNEL ?
                                         "mode/" : "umode/") + name, this);
}

ModeAction SimpleUserModeHandler::OnModeChange(User* source, User* dest,
...
ModeAction SimpleChannelModeHandler::OnModeChange(User* source, User* dest,
        Channel* channel, std::string &parameter, bool adding) {
    /* We're either trying to add a mode we already have or
        remove a mode we don't have, deny. */
...
ModeWatcher::ModeWatcher(Module* Creator, const std::string& modename,
                         ModeType type)
...
PrefixMode::PrefixMode(Module* Creator, const std::string& Name,
                       char ModeLetter, unsigned int Rank, char PrefixChar)
...
ModResult PrefixMode::AccessCheck(User* src, Channel*, std::string& value,
                                  bool adding) {
...
ModeAction PrefixMode::OnModeChange(User* source, User*, Channel* chan,
...
void PrefixMode::Update(unsigned int rank, unsigned int setrank,
                        unsigned int unsetrank, bool selfrm) {
    prefixrank = rank;
    ranktoset = setrank;
...
ModeAction ParamModeBase::OnModeChange(User* source, User*, Channel* chan,
                                       std::string& parameter, bool adding) {
    if (adding) {
...
ModeAction ModeParser::TryMode(User* user, User* targetuser, Channel* chan,
                               Modes::Change& mcitem, bool SkipACL) {
    ...
    if (chan && !SkipACL && (MOD_RESULT != MOD_RES_ALLOW)) {
        ...
        if (MOD_RESULT == MOD_RES_PASSTHRU) {
            ...
            if (ourrank < neededrank) {
                user->WriteNumeric(Numerics::ChannelPrivilegesNeeded(chan, neededrank,
                                   InspIRCd::Format("%s channel mode %c (%s)",
    ...
    std::pair<ModeWatcherMap::iterator, ModeWatcherMap::iterator> itpair =
        modewatchermap.equal_range(mh->name);
    ...
    if ((chan || (!chan && adding)) && IS_LOCAL(user) && mh->NeedsOper()
            && !user->HasModePermission(mh)) {
        /* It's an oper only mode, and they don't have access to it. */
        if (user->IsOper()) {
            user->WriteNumeric(ERR_NOPRIVILEGES,
                               InspIRCd::Format("Permission Denied - Oper type %s does not have access to %sset %s mode %c",
                                                user->oper->name.c_str(), adding ? "" : "un",
                                                type == MODETYPE_CHANNEL ? "channel" : "user", modechar));
        } else {
            user->WriteNumeric(ERR_NOPRIVILEGES,
                               InspIRCd::Format("Permission Denied - Only operators may %sset %s mode %c",
...
void ModeParser::ModeParamsToChangeList(User* user, ModeType type,
                                        const std::vector<std::string>& parameters, Modes::ChangeList& changelist,
    ...
    for (std::string::const_iterator letter = mode_sequence.begin();
            letter != mode_sequence.end(); letter++) {
        ...
        if (!mh) {
            ...
            user->WriteNumeric(numeric, modechar,
...
static bool IsModeParamValid(User* user, Channel* targetchannel,
                             User* targetuser, const Modes::Change& item) {
...
void ModeParser::Process(User* user, Channel* targetchannel, User* targetuser,
                         Modes::ChangeList& changelist, ModeProcessFlag flags) {
    ...
    do {
        unsigned int n = ProcessSingle(user, targetchannel, targetuser, changelist,
                                       flags, processed);
        processed += n;
    } while (processed < changelist.size());
}

unsigned int ModeParser::ProcessSingle(User* user, Channel* targetchannel,
                                       User* targetuser, Modes::ChangeList& changelist, ModeProcessFlag flags,
                                       unsigned int beginindex) {
    LastChangeList.clear();

    unsigned int modes_processed = 0;
    Modes::ChangeList::List& list = changelist.getlist();
    for (Modes::ChangeList::List::iterator i = list.begin()+beginindex;
            i != list.end(); ++i) {
        modes_processed++;
        ...
        ModeAction ma = TryMode(user, targetuser, targetchannel, item,
    ...
    if (!LastChangeList.empty()) {
        ClientProtocol::Events::Mode modeevent(user, targetchannel, targetuser,
                                               LastChangeList);
        if (targetchannel) {
...
void ModeParser::CleanMask(std::string &mask) {
    ...
    std::string::size_type pos_of_colons =
        mask.find("::"); /* Because ipv6 addresses are colon delimited -- double so it treats extban as nick */
    ...
    if ((pos_of_pling == std::string::npos) && (pos_of_at == std::string::npos)) {
        ...
        if ((pos_of_dot == std::string::npos) && (pos_of_colons == std::string::npos)
                && mask[0] != ':') {
    ...
    } else if ((pos_of_pling == std::string::npos)
               && (pos_of_at != std::string::npos)) {
...
void ModeParser::AddMode(ModeHandler* mh) {
    ...
    if (pm) {
        if ((pm->GetPrefix() > 126) || (pm->GetPrefix() == ',')
                || (pm->GetPrefix() == ':') || (pm->GetPrefix() == '#'))
        ...
        if (otherpm)
            throw ModuleException(
                InspIRCd::Format("Mode prefix for %s already used by %s from %s: %c",
                                 mh->name.c_str(), otherpm->name.c_str(),
                                 otherpm->creator->ModuleSourceFile.c_str(), pm->GetPrefix()));
    }

    ModeHandler*& slot = modehandlers[mh->GetModeType()][mh->GetModeChar()-65];
    if (slot)
        throw ModuleException(
            InspIRCd::Format("Mode letter for %s already used by %s from %s: %c",
                             mh->name.c_str(), slot->name.c_str(), slot->creator->ModuleSourceFile.c_str(),
                             mh->GetModeChar()));
    ...
    if ((mh->GetModeType() == MODETYPE_USER) || (mh->IsParameterMode())
            || (!mh->IsListMode())) {
        modeid = AllocateModeId(mh->GetModeType());
    }

    std::pair<ModeHandlerMap::iterator, bool> res =
        modehandlersbyname[mh->GetModeType()].insert(std::make_pair(mh->name, mh));
    if (!res.second) {
        ModeHandler* othermh = res.first->second;
        throw ModuleException(
            InspIRCd::Format("Mode name %s already used by %c from %s",
                             mh->name.c_str(), othermh->GetModeChar(),
                             othermh->creator->ModuleSourceFile.c_str()));
    }
...
bool ModeParser::DelMode(ModeHandler* mh) {
    ...
    case MODETYPE_CHANNEL: {
        ...
        for (chan_hash::const_iterator i = chans.begin(); i != chans.end(); ) {
            ...
            this->Process(ServerInstance->FakeClient, chan, NULL, changelist,
                          MODE_LOCALONLY);
        }
    }
    break;
    ...
    slot = NULL;
    if (mh->IsPrefixMode()) {
        mhlist.prefix.erase(std::find(mhlist.prefix.begin(), mhlist.prefix.end(),
                                      mh->IsPrefixMode()));
    } else if (mh->IsListModeBase()) {
        mhlist.list.erase(std::find(mhlist.list.begin(), mhlist.list.end(),
                                    mh->IsListModeBase()));
    }
    return true;
}
...
PrefixMode* ModeParser::FindNearestPrefixMode(unsigned int rank) {
    ...
    for (PrefixModeList::const_iterator i = prefixmodes.begin();
            i != prefixmodes.end(); ++i) {
        PrefixMode* thispm = *i;
...
std::string ModeParser::BuildPrefixes(bool lettersAndModes) {
    ...
    for (std::vector<PrefixMode*>::const_reverse_iterator n = prefixes.rbegin();
            n != prefixes.rend(); ++n) {
...
bool ModeParser::DelModeWatcher(ModeWatcher* mw) {
    std::pair<ModeWatcherMap::iterator, ModeWatcherMap::iterator> itpair =
        modewatchermap.equal_range(mw->GetModeName());
    for (ModeWatcherMap::iterator i = itpair.first; i != itpair.second; ++i) {
        if (i->second == mw) {
