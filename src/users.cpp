...
User::User(const std::string& uid, Server* srv, UserType type)
    ...
    ServerInstance->Logs->Log("USERS", LOG_DEBUG, "New UUID for user: %s",
                              uuid.c_str());
...
LocalUser::LocalUser(int myfd, irc::sockets::sockaddrs* client,
    ...
    : User(ServerInstance->UIDGen.GetUID(), ServerInstance->FakeClient->server,
           USERTYPE_LOCAL)
    , eh(this)
    , serializer(NULL)
    , bytes_in(0)
...
bool LocalUser::HasModePermission(const ModeHandler* mh) const {
    ...
    return ((mh->GetModeType() == MODETYPE_USER ? oper->AllowedUserModes :
...
void UserIOHandler::OnDataReady() {
    ...
    if (recvq.length() > user->MyClass->GetRecvqMax()
        ...
        ServerInstance->SNO->WriteToSnoMask('a',
                                            "User %s RecvQ of %lu exceeds connect class maximum of %lu",
                                            user->nick.c_str(), (unsigned long)recvq.length(),
                                            user->MyClass->GetRecvqMax());
        return;
    }
...
void UserIOHandler::AddWriteBuf(const std::string &data) {
    ...
    if (!user->quitting
            && getSendQSize() + data.length() > user->MyClass->GetSendqHardMax() &&
            !user->HasPrivPermission("users/flood/increased-buffers")) {
...
bool UserIOHandler::OnSetEndPoint(const irc::sockets::sockaddrs& server,
                                  const irc::sockets::sockaddrs& client) {
...
void User::Oper(OperInfo* info) {
    ...
    if (localuser && opermh) {
        ...
        ClientProtocol::Events::Mode modemsg(ServerInstance->FakeClient, NULL,
                                             localuser, changelist);
    ...
    ServerInstance->SNO->WriteToSnoMask('o',
                                        "%s (%s@%s) is now a server operator of type %s (using oper '%s')",
                                        nick.c_str(), ident.c_str(), GetRealHost().c_str(), oper->name.c_str(),
                                        opername.c_str());
    this->WriteNumeric(RPL_YOUAREOPER, InspIRCd::Format("You are now %s %s",
                       strchr("aeiouAEIOU", oper->name[0]) ? "an" : "a", oper->name.c_str()));
...
namespace {
bool ParseModeList(std::bitset<64>& modeset, ConfigTag* tag,
    ...
    for (std::string::const_iterator iter = modes.begin(); iter != modes.end();
        ...
        if (chr == '*') {
            ...
            ServerInstance->Logs->Log("CONFIG", LOG_DEFAULT,
                                      "'%c' is not a valid value for <class:%s>, ignoring...", chr, field.c_str());
        }
    }
    return hasmodes;
}
...
void OperInfo::init() {
    ...
    for(std::vector<reference<ConfigTag> >::iterator iter = class_blocks.begin();
            iter != class_blocks.end(); ++iter) {
        ConfigTag* tag = *iter;
...
void User::UnOper() {
    ...
    Modes::ChangeList changelist;
    const ModeParser::ModeHandlerMap& usermodes = ServerInstance->Modes->GetModes(
                MODETYPE_USER);
    for (ModeParser::ModeHandlerMap::const_iterator i = usermodes.begin();
            i != usermodes.end(); ++i) {
...
void LocalUser::CheckClass(bool clone_count) {
    ...
    if (!a) {
        ...
        ServerInstance->Users->QuitUser(this, a->config->getString("reason",
                                        "Unauthorised connection", 1));
        return;
    } else if (clone_count) {
        const UserManager::CloneCounts& clonecounts =
            ServerInstance->Users->GetCloneCounts(this);
        if ((a->GetMaxLocal()) && (clonecounts.local > a->GetMaxLocal())) {
            ServerInstance->Users->QuitUser(this,
                                            "No more connections allowed from your host via this connect class (local)");
            if (a->maxconnwarn) {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "WARNING: maximum local connections for the %s class (%ld) exceeded by %s",
                                                    a->name.c_str(), a->GetMaxLocal(), this->GetIPString().c_str());
            }
            return;
        } else if ((a->GetMaxGlobal()) && (clonecounts.global > a->GetMaxGlobal())) {
            ServerInstance->Users->QuitUser(this,
                                            "No more connections allowed from your host via this connect class (global)");
            if (a->maxconnwarn) {
                ServerInstance->SNO->WriteToSnoMask('a',
                                                    "WARNING: maximum global connections for the %s class (%ld) exceeded by %s",
...
void LocalUser::FullConnect() {
    ...
    ServerInstance->SNO->WriteToSnoMask('c',
                                        "Client connecting on port %d (class %s): %s (%s) [%s\x0F]",
                                        this->server_sa.port(), this->MyClass->name.c_str(), GetFullRealHost().c_str(),
                                        this->GetIPString().c_str(), this->GetRealName().c_str());
    ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                              "BanCache: Adding NEGATIVE hit for " + this->GetIPString());
...
bool User::ChangeNick(const std::string& newnick, time_t newts) {
    if (quitting) {
        ServerInstance->Logs->Log("USERS", LOG_DEFAULT,
                                  "ERROR: Attempted to change nick of a quitting user: " + this->nick);
        return false;
    }
...
bool User::SetClientIP(const std::string& address) {
    ...
    if (!irc::sockets::aptosa(address,
                              client_sa.family() == AF_UNSPEC ? 0 : client_sa.port(), sa)) {
        return false;
    }
...
void LocalUser::Write(const ClientProtocol::SerializedMessage& text) {
    ...
    if (ServerInstance->Config->RawLog) {
        ...
        ServerInstance->Logs->Log("USEROUTPUT", LOG_RAWIO, "C[%s] O %.*s", uuid.c_str(),
                                  (int) nlpos, text.c_str());
    }
...
void LocalUser::Send(ClientProtocol::Event& protoev) {
    if (!serializer) {
        ServerInstance->Logs->Log("USERS", LOG_DEBUG,
                                  "BUG: LocalUser::Send() called on %s who does not have a serializer!",
...
void LocalUser::Send(ClientProtocol::Event& protoev,
    ...
    for (ClientProtocol::MessageList::const_iterator i = msglist.begin();
            i != msglist.end(); ++i) {
...
already_sent_t User::ForEachNeighbor(ForEachNeighborHandler& handler,
                                     bool include_self) {
    ...
    for (std::map<User*, bool>::const_iterator i = exceptions.begin();
            i != exceptions.end(); ++i) {
    ...
    for (IncludeChanList::const_iterator i = include_chans.begin();
        ...
        for (Channel::MemberMap::const_iterator j = userlist.begin();
                j != userlist.end(); ++j) {
...
bool User::ChangeDisplayedHost(const std::string& shost) {
    ...
    if (IS_LOCAL(this) && this->registered != REG_NONE) {
        this->WriteNumeric(RPL_YOURDISPLAYEDHOST, this->GetDisplayedHost(),
    ...
    if (!explicit_name.empty()) {
        for (ServerConfig::ClassVector::const_iterator i =
            ...
            if (explicit_name == c->name) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "Connect class explicitly set to %s",
                                          explicit_name.c_str());
                found = c;
            }
        }
        ...
        for (ServerConfig::ClassVector::const_iterator i =
            ...
            if (MOD_RESULT == MOD_RES_ALLOW) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class was explicitly chosen by a module",
                                          c->GetName().c_str());
                found = c;
                break;
            }
            ...
            if (c->type == CC_NAMED) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as neither <connect:allow> nor <connect:deny> are set",
                                          c->GetName().c_str());
                continue;
            }
            ...
            if (c->config->getBool("registered", regdone) != regdone) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as it requires that the user is %s",
                                          c->GetName().c_str(), regdone ? "not fully connected" : "fully connected");
                continue;
            }

            bool hostmatches = false;
            for (std::vector<std::string>::const_iterator host = c->GetHosts().begin();
                    host != c->GetHosts().end(); ++host) {
                if (InspIRCd::MatchCIDR(this->GetIPString(), *host)
                        || InspIRCd::MatchCIDR(this->GetRealHost(), *host)) {
                    hostmatches = true;
                    break;
                }
            }
            if (!hostmatches) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as neither the host (%s) nor the IP (%s) matches %s",
                                          c->GetName().c_str(), this->GetRealHost().c_str(), this->GetIPString().c_str(),
                                          c->GetHost().c_str());
                continue;
            }
            ...
            if (c->limit && (c->GetReferenceCount() >= c->limit)) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as it has reached its user limit (%lu)",
            ...
            if (!c->ports.empty() && !c->ports.count(this->server_sa.port())) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as the connection port (%d) is not any of %s",
                                          c->GetName().c_str(), this->server_sa.port(),
                                          stdalgo::string::join(c->ports).c_str());
                continue;
            }

            if (regdone && !c->password.empty()
                    && !ServerInstance->PassCompare(this, c->password, password, c->passwordhash)) {
                ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                          "The %s connect class is not suitable as requires a password and %s",
                                          c->GetName().c_str(), password.empty() ? "one was not provided" :
            ...
            ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                      "The %s connect class is suitable for %s (%s)",
...
void User::WriteNotice(const std::string& text) {
    ...
    ClientProtocol::Messages::Privmsg msg(ClientProtocol::Messages::Privmsg::nocopy,
                                          ServerInstance->FakeClient, localuser, text, MSG_NOTICE);
...
ConnectClass::ConnectClass(ConfigTag* tag, char t, const std::string& mask,
    ...
    for (ConfigItems::const_iterator piter = parentkeys.begin();
            piter != parentkeys.end(); ++piter) {
        // The class name and parent name are not inherited
        if (stdalgo::string::equalsci(piter->first, "name")
                || stdalgo::string::equalsci(piter->first, "parent")) {
            continue;
        }
    ...
    for (ConfigItems::const_iterator citer = childkeys.begin();
            citer != childkeys.end(); ++citer) {
