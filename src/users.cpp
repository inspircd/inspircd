...
User::User(const std::string &uid, const std::string& sid, int type)
	...
	dns_done(0), quietquit(0), quitting(0), quitting_sendq(0), exempt(0), lastping(0),
...
{
	ServerInstance->DoWhois(src, this, signon, ServerInstance->Time() - idle_lastmsg);
}
...
void LocalUser::FullConnect()
{
	...
	ServerInstance->SNO->WriteToSnoMask('c',"Client connecting on port %d (class %s): %s!%s@%s (%s) [%s]",
		this->GetServerPort(), this->MyClass->name.c_str(), this->nick.c_str(), this->ident.c_str(), this->host.c_str(), this->fullname.c_str(), this->GetIPString());
