	...
	, already_sent(0)
{
	...
	host = insp::CloakedString(GetIPString());
}
...
const std::string& User::MakeHost()
{
	...
	this->cached_makehost = ident + "@" + host.GetReal();
	return this->cached_makehost;
}
...
const std::string& User::GetFullHost()
{
	...
	// XXX: Is there really a need to cache this?
	this->cached_fullhost = nick + "!" + ident + "@" + host.GetDisplay();
	return this->cached_fullhost;
}
...
const std::string& User::GetFullRealHost()
{
	...
	// XXX: Is there really a need to cache this?
	this->cached_fullrealhost = nick + "!" + ident + "@" + host.GetReal();
	return this->cached_fullrealhost;
}
...
void User::Oper(OperInfo* info)
{
	...
	ServerInstance->SNO->WriteToSnoMask('o',"%s (%s@%s) is now an IRC operator of type %s (using oper '%s')",
		nick.c_str(), ident.c_str(), host.GetReal().c_str(), oper->name.c_str(), opername.c_str());
...
bool User::ChangeDisplayedHost(const std::string& shost)
{
	...
	this->host.SetDisplay(shost.substr(ServerInstance->Config->Limits.MaxHost));
	this->InvalidateCache();

	if (IS_LOCAL(this))
		this->WriteNumeric(RPL_YOURDISPLAYEDHOST, this->host.GetDisplay(), "is now your displayed host");
	...
	else
	{
		for (ServerConfig::ClassVector::const_iterator i = ServerInstance->Config->Classes.begin(); i != ServerInstance->Config->Classes.end(); ++i)
		{
			...
			if (!InspIRCd::MatchCIDR(this->GetIPString(), c->GetHost(), NULL) &&
			    !InspIRCd::MatchCIDR(this->host.GetReal(), c->GetHost(), NULL))
			{
