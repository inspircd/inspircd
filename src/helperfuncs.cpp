...
void InspIRCd::ProcessColors(file_cache& input) {
    ...
    for(file_cache::iterator it = input.begin(), it_end = input.end(); it != it_end;
            it++) {
        std::string ret = *it;
        for(int i = 0; special[i].character.empty() == false; ++i) {
            ...
            while(pos != std::string::npos) {
                ret = ret.substr(0, pos) + special[i].replace + ret.substr(
                          pos + special[i].character.size());
...
bool InspIRCd::DefaultIsChannel(const std::string& chname) {
    if (chname.empty()
            || chname.length() > ServerInstance->Config->Limits.ChanMax) {
        return false;
    }
...
bool InspIRCd::IsHost(const std::string& host) {
    ...
    for (std::string::const_iterator iter = host.begin(); iter != host.end();
            ++iter) {
...
bool InspIRCd::IsValidDuration(const std::string& duration) {
    for (std::string::const_iterator i = duration.begin(); i != duration.end();
            ++i) {
        unsigned char c = *i;
        if (((c >= '0') && (c <= '9'))) {
            continue;
...
std::string InspIRCd::Format(va_list& vaList, const char* formatString) {
    ...
    while (true) {
        ...
        int vsnret = vsnprintf(&formatBuffer[0], formatBuffer.size(), formatString,
                               dst);
        va_end(dst);
