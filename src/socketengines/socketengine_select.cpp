...
void SocketEngine::OnSetEvent(EventHandler* eh, int old_mask, int new_mask) {
    ...
    if (diff & (FD_WANT_POLL_WRITE | FD_WANT_FAST_WRITE | FD_WANT_SINGLE_WRITE)) {
        if (new_mask & (FD_WANT_POLL_WRITE | FD_WANT_FAST_WRITE |
...
int SocketEngine::DispatchEvents() {
    ...
    for (int i = 0, j = sresult; i <= MaxFD && j > 0; i++) {
        int has_read = FD_ISSET(i, &rfdset), has_write = FD_ISSET(i, &wfdset),
            has_error = FD_ISSET(i, &errfdset);
        ...
        if (has_write) {
            int newmask = (ev->GetEventMask() & ~(FD_WRITE_WILL_BLOCK |
                                                  FD_WANT_SINGLE_WRITE));
