...
BanCacheHit::BanCacheHit(const std::string& type, const std::string& reason,
                         time_t seconds)
    : Type(type)
    , Reason(reason)
...
BanCacheHit *BanCacheManager::AddHit(const std::string &ip,
                                     const std::string &type, const std::string &reason, time_t seconds) {
...
bool BanCacheManager::RemoveIfExpired(BanCacheHash::iterator& it) {
    ...
    ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                              "Hit on " + it->first + " is out of date, removing!");
    delete it->second;
    it = BanHash.erase(it);
    return true;
}
...
void BanCacheManager::RemoveEntries(const std::string& type, bool positive) {
    if (positive) {
        ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                                  "BanCacheManager::RemoveEntries(): Removing positive hits for " + type);
    } else {
        ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                                  "BanCacheManager::RemoveEntries(): Removing all negative hits");
    }
