...
void CheckPingTimeout(LocalUser* user) {
    ...
    if (!user->lastping) {
        ...
        time_t secs = ServerInstance->Time() - (user->nextping -
                                                user->MyClass->GetPingTime());
        const std::string message = "Ping timeout: " + ConvToStr(secs) +
...
void CheckRegistrationTimeout(LocalUser* user) {
    if (user->GetClass()
            && (ServerInstance->Time() > (user->signon +
                                          user->GetClass()->GetRegTimeout()))) {
...
void UserManager::AddUser(int socket, ListenSocket* via,
    ...
    if (!SocketEngine::AddFd(eh, FD_WANT_FAST_READ | FD_WANT_EDGE_WRITE)) {
        ServerInstance->Logs->Log("USERS", LOG_DEBUG,
                                  "Internal error on new connection");
    ...
    for (ListenSocket::IOHookProvList::iterator i = via->iohookprovs.begin();
        ...
        if (!iohookprovref) {
            ...
            ServerInstance->Logs->Log("USERS", LOG_DEFAULT,
                                      "Non-existent I/O hook '%s' in <bind:%s> tag at %s",
                                      iohookprovref.GetProvider().c_str(), hooktype,
                                      via->bind_tag->getTagLocation().c_str());
            this->QuitUser(New,
                           InspIRCd::Format("Internal error handling connection (misconfigured %s)",
                                            hooktype));
            return;
        }
    ...
    if (this->local_users.size() > ServerInstance->Config->SoftLimit) {
        ServerInstance->SNO->WriteToSnoMask('a',
                                            "Warning: softlimit value has been reached: %d clients",
                                            ServerInstance->Config->SoftLimit);
    ...
    if (b) {
        if (!b->Type.empty() && !New->exempt) {
            ...
            ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                                      "BanCache: Positive hit for " + New->GetIPString());
            ...
            ServerInstance->Logs->Log("BANCACHE", LOG_DEBUG,
                                      "BanCache: Negative hit for " + New->GetIPString());
        }
    } else {
        if (!New->exempt) {
...
void UserManager::QuitUser(User* user, const std::string& quitmessage,
                           const std::string* operquitmessage) {
    if (user->quitting) {
        ServerInstance->Logs->Log("USERS", LOG_DEFAULT,
                                  "ERROR: Tried to quit quitting user: " + user->nick);
        return;
    }

    if (IS_SERVER(user)) {
        ServerInstance->Logs->Log("USERS", LOG_DEFAULT,
                                  "ERROR: Tried to quit server user: " + user->nick);
        return;
    }
    ...
    user->quitting = true;
    ServerInstance->Logs->Log("USERS", LOG_DEBUG, "QuitUser: %s=%s '%s'",
                              user->uuid.c_str(), user->nick.c_str(), quitmessage.c_str());
    if (localuser) {
        ClientProtocol::Messages::Error errormsg(
            InspIRCd::Format("Closing link: (%s@%s) [%s]", user->ident.c_str(),
                             user->GetRealHost().c_str(), operquitmsg.c_str()));
    ...
    if (IS_LOCAL(user)) {
        ...
        if (lu->registered == REG_ALL) {
            ServerInstance->SNO->WriteToSnoMask('q',"Client exiting: %s (%s) [%s]",
                                                user->GetFullRealHost().c_str(), user->GetIPString().c_str(),
                                                operquitmsg.c_str());
        }
    ...
    if (!clientlist.erase(user->nick)) {
        ServerInstance->Logs->Log("USERS", LOG_DEFAULT,
                                  "ERROR: Nick not found in clientlist, cannot remove: " + user->nick);
    }
...
void UserManager::ServerNoticeAll(const char* text, ...) {
    ...
    ClientProtocol::Messages::Privmsg msg(ClientProtocol::Messages::Privmsg::nocopy,
                                          ServerInstance->FakeClient, ServerInstance->Config->GetServerName(), message,
                                          MSG_NOTICE);
    ClientProtocol::Event msgevent(ServerInstance->GetRFCEvents().privmsg, msg);

    for (LocalList::const_iterator i = local_users.begin(); i != local_users.end();
            ++i) {
        LocalUser* user = *i;
        user->Send(msgevent);
    }
