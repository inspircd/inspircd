...
class UserResolver
  ...
  protected:
    ...
    UserResolver(DNS::Manager* mgr, Module* me, LocalUser* user,
                 const std::string& to_resolve, DNS::QueryType qt)
    ...
    static void HandleError(LocalUser* user, const std::string& message) {
        user->WriteNotice("*** " + message + "; using your IP address (" +
                          user->GetIPString() + ") instead.");
  ...
  public:
    void OnError(const DNS::Query* query) CXX11_OVERRIDE {
        ...
        if (user && user->client_sa == sa) {
            HandleError(user, "Could not resolve your hostname: " +
                        this->manager->GetErrorStr(query->error));
        }
    }
};
...
class UserIPResolver CXX11_FINAL
    : public UserResolver {
  public:
    UserIPResolver(DNS::Manager* mgr, Module* me, LocalUser* user,
                   const std::string& host)
        : UserResolver(mgr, me, user, host,
                       user->client_sa.family() == AF_INET6 ? DNS::QUERY_AAAA : DNS::QUERY_A) {
    }
    ...
    void OnLookupComplete(const DNS::Query* query) CXX11_OVERRIDE {
        ...
        if (matches) {
            ...
            user->WriteNotice("*** Found your hostname (" + this->question.name +
                              (query->cached ? ") -- cached" : ")"));
            bool display_is_real = user->GetDisplayedHost() == user->GetRealHost();
            ...
            HandleError(user, "Could not resolve your hostname: No " +
                        this->manager->GetTypeStr(this->question.type) + " records found");
        }
    }
};
