...
CmdResult CommandQline::Handle(User* user, const Params& parameters) {
    ...
    if (parameters.size() >= 3) {
        NickMatcher matcher;
        if (InsaneBan::MatchesEveryone(parameters[0], matcher, user, 'Q',
                                       "nickmasks")) {
            return CMD_FAILURE;
        }

        if (parameters[0].find('@') != std::string::npos
                || parameters[0].find('!') != std::string::npos
                || parameters[0].find('.') != std::string::npos) {
        ...
        QLine* ql = new QLine(ServerInstance->Time(), duration, user->nick,
                              parameters[2], parameters[0]);
        if (ServerInstance->XLines->AddLine(ql,user)) {
            if (!duration) {
                ServerInstance->SNO->WriteToSnoMask('x',
                                                    "%s added a permanent Q-line on %s: %s", user->nick.c_str(),
                                                    parameters[0].c_str(), parameters[2].c_str());
            } else {
                ServerInstance->SNO->WriteToSnoMask('x',
                                                    "%s added a timed Q-line on %s, expires in %s (on %s): %s",
        ...
        if (ServerInstance->XLines->DelLine(parameters[0].c_str(), "Q", reason, user)) {
            ServerInstance->SNO->WriteToSnoMask('x', "%s removed Q-line on %s: %s",
...
bool CommandQline::NickMatcher::Check(User* user,
