...
CmdResult CommandInvite::Handle(User* user, const Params& parameters) {
    ...
    if (parameters.size() >= 2) {
        ...
        if (c->HasUser(u)) {
            user->WriteNumeric(ERR_USERONCHANNEL, u->nick, c->name,
        ...
        if (invapi.announceinvites != Invite::ANNOUNCE_NONE) {
            excepts.insert(user);
            ClientProtocol::Messages::Privmsg privmsg(ServerInstance->FakeClient, c,
                    InspIRCd::Format("*** %s invited %s into the channel", user->nick.c_str(),
                                     u->nick.c_str()), MSG_NOTICE);
...
RouteDescriptor CommandInvite::GetRouting(User* user,
