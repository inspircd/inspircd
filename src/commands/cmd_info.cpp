...
CmdResult CommandInfo::Handle (const std::vector<std::string>&, User *user)
{
	...
	user->WriteNumeric(RPL_INFO, "%s :    Majic          MacGyver        danieldg        Namegduf", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s :    Trystan        Phoenix", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s : ", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s :\2Other Contributors\2:", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s :   dmb             Zaba            skenmy         Skip", user->nick.c_str());
	...
	user->WriteNumeric(RPL_INFO, "%s :   GreenReaper", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s : ", user->nick.c_str());
	...
	user->WriteNumeric(RPL_INFO, "%s :   Dan             LeaChim", user->nick.c_str());
	user->WriteNumeric(RPL_INFO, "%s : ", user->nick.c_str());
