...
CmdResult CommandGline::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	if (parameters.size() >= 3)
	{
		...
		if (ServerInstance->XLines->AddLine(gl, user))
		{
			if (!duration)
			{
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline,"%s added permanent G-line for %s: %s",user->nick.c_str(),target.c_str(), parameters[2].c_str());
			}
			else
			{
				time_t c_requires_crap = duration + ServerInstance->Time();
				std::string timestr = InspIRCd::TimeString(c_requires_crap);
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline,"%s added timed G-line for %s, expires on %s: %s",user->nick.c_str(),target.c_str(),
	...
	else
	{
		...
		{
			SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline,"%s removed G-line on %s",user->nick.c_str(),target.c_str());
		}
		else
		{
