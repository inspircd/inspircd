...
bool TestSuite::RealGenerateUIDTests()
{
	...
	std::string correct_uid = std::string(ServerInstance->Config->sid, 3) + std::string(UUID_LENGTH - 4, 'A');
	...
	correct_uid = std::string(ServerInstance->Config->sid, 3) + "0" + std::string(UUID_LENGTH - 5, 'A');
