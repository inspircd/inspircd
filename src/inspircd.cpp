...
void CheckRoot() {
    ...
    std::cout << con_red << "Warning!" << con_reset <<
              " You have started as root. Running as root is generally not required" <<
              std::endl
    ...
    if (isatty(fileno(stdout))) {
        std::cout <<
                  ...
                  << std::endl
                  << "then you can pass the " << con_bright << "--runasroot" << con_reset <<
                  " option to disable this wait." << std::endl;
        sleep(30);
    } else {
        std::cout <<
                  "If you are sure that you need to run as root then you can pass the " <<
                  con_bright << "--runasroot" << con_reset << std::endl
...
void CollectStats() {
    ...
    ServerInstance->stats.LastCPU.dwHighDateTime = KernelTime.dwHighDateTime +
            UserTime.dwHighDateTime;
    ServerInstance->stats.LastCPU.dwLowDateTime = KernelTime.dwLowDateTime +
...
void CheckTimeSkip(time_t oldtime, time_t newtime) {
    ...
    if (timediff > ServerInstance->Config->TimeSkipWarn) {
        ServerInstance->SNO->WriteToSnoMask('a',
                                            "\002Performance warning!\002 Server clock jumped forwards by %lu seconds!",
                                            timediff);
    }

    else if (timediff < -ServerInstance->Config->TimeSkipWarn) {
        ServerInstance->SNO->WriteToSnoMask('a',
                                            "\002Performance warning!\002 Server clock jumped backwards by %lu seconds!",
                                            labs(timediff));
    }
}
...
void DropRoot() {
    ...
    if (!SetGroup.empty()) {
        ...
        if (setgroups(0, NULL) == -1) {
            ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                      "setgroups() failed (wtf?): %s", strerror(errno));
        ...
        if (!g) {
            ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                      "getgrnam(%s) failed (wrong group?): %s", SetGroup.c_str(), strerror(errno));
            InspIRCd::QuickExit(EXIT_STATUS_CONFIG);
        }

        if (setgid(g->gr_gid) == -1) {
            ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                      "setgid(%d) failed (wrong group?): %s", g->gr_gid, strerror(errno));
            InspIRCd::QuickExit(EXIT_STATUS_CONFIG);
        }
    }
    ...
    if (!SetUser.empty()) {
        ...
        if (!u) {
            ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                      "getpwnam(%s) failed (wrong user?): %s", SetUser.c_str(), strerror(errno));
            InspIRCd::QuickExit(EXIT_STATUS_CONFIG);
        }

        if (setuid(u->pw_uid) == -1) {
            ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                      "setuid(%d) failed (wrong user?): %s", u->pw_uid, strerror(errno));
            InspIRCd::QuickExit(EXIT_STATUS_CONFIG);
        }
    }
#endif
...
void ForkIntoBackground() {
    ...
    int childpid = fork();
    if (childpid < 0) {
        ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT, "fork() failed: %s",
                                  strerror(errno));
        std::cout << con_red << "Error:" << con_reset <<
                  " unable to fork into background: " << strerror(errno);
...
void IncreaseCoreDumpSize() {
    ...
    if (getrlimit(RLIMIT_CORE, &rl) == -1) {
        ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                  "Unable to increase core dump size: getrlimit(RLIMIT_CORE) failed: %s",
                                  strerror(errno));
        return;
    }
    ...
    if (setrlimit(RLIMIT_CORE, &rl) == -1) {
        ServerInstance->Logs->Log("STARTUP", LOG_DEFAULT,
                                  "Unable to increase core dump size: setrlimit(RLIMIT_CORE) failed: %s",
                                  strerror(errno));
    }
#endif
}
...
void ParseOptions() {
    ...
    int ret;
    while ((ret = getopt_long(ServerInstance->Config->cmdline.argc, argv, ":c:",
                              longopts, NULL)) != -1) {
        switch (ret) {
        case 0:
        ...
        default:
            ...
            std::cout << con_red << "Error:" <<  con_reset << " unknown option '" <<
                      argv[optind-1] << "'." << std::endl
                      << con_bright << "Usage: " << con_reset << argv[0] <<
                      " [--config <file>] [--debug] [--nofork] [--nolog]" << std::endl
                      << std::string(strlen(argv[0]) + 8,
                                     ' ') << "[--nopid] [--runasroot] [--version]" << std::endl;
...
void TryBindPorts() {
    ...
    if (!pl.empty()) {
        std::cout << con_red << "Warning!" << con_reset << " Some of your listener" <<
                  (pl.size() == 1 ? "s" : "") << " failed to bind:" << std::endl
                  << std::endl;

        for (FailedPortList::const_iterator iter = pl.begin(); iter != pl.end();
                ++iter) {
            const FailedPort& fp = *iter;
            std::cout << "  " << con_bright << fp.sa.str() << con_reset << ": " << strerror(
                          fp.error) << '.' << std::endl
        ...
        std::cout << con_bright << "Hints:" << con_reset << std::endl
                  << "- For TCP/IP listeners try using a public IP address in <bind:address> instead"
                  << std::endl
                  << "  of * or leaving it blank." << std::endl
                  << "- For UNIX socket listeners try enabling <bind:rewrite> to replace old sockets."
                  << std::endl;
    }
}
...
void InspIRCd::WritePID(const std::string& filename, bool exitonfail) {
    ...
    if (!ServerInstance->Config->cmdline.writepid) {
        this->Logs->Log("STARTUP", LOG_DEFAULT,
                        "--nopid specified on command line; PID file not written.");
        return;
    }

    std::string fname = ServerInstance->Config->Paths.PrependRuntime(
                            filename.empty() ? "inspircd.pid" : filename);
    ...
    if (outfile.is_open()) {
        ...
        if (exitonfail) {
            std::cout << "Failed to write PID-file '" << fname << "', exiting." <<
                      std::endl;
        }
        this->Logs->Log("STARTUP", LOG_DEFAULT, "Failed to write PID-file '%s'%s",
                        fname.c_str(), (exitonfail ? ", exiting." : ""));
        if (exitonfail) {
...
InspIRCd::InspIRCd(int argc, char** argv)
    ...
    if (!FindConfigFile(ConfigFileName)) {
        this->Logs->Log("STARTUP", LOG_DEFAULT, "Unable to open config file %s",
                        ConfigFileName.c_str());
        std::cout << "ERROR: Cannot open config file: " << ConfigFileName << std::endl
                  << "Exiting..." << std::endl;
    ...
    std::cout << "InspIRCd Process ID: " << con_green << getpid() << con_reset <<
              std::endl;
    ...
    if (Config->sid.empty()) {
        Config->sid = UIDGenerator::GenerateSID(Config->ServerName,
                                                Config->ServerDesc);
    }
    ...
    std::cout << "InspIRCd is now running as '" << Config->ServerName << "'[" <<
              Config->GetSID() << "] with " << SocketEngine::GetMaxFds() <<
    ...
    if (!Config->cmdline.nofork) {
        if (kill(getppid(), SIGTERM) == -1) {
            ...
            Logs->Log("STARTUP", LOG_DEFAULT, "Error killing parent process: %s",
                      strerror(errno));
        }
    }
    ...
    if ((!Config->cmdline.nofork) && (!Config->cmdline.forcedebug)) {
        ...
        Logs->Log("STARTUP", LOG_DEFAULT,
                  "Keeping pseudo-tty open as we are running in the foreground.");
    }
#else
    ...
    Logs->Log("STARTUP", LOG_DEFAULT,
              "Startup complete as '%s'[%s], %lu max open sockets",
...
void InspIRCd::Run() {
    ...
    while (true) {
        ...
        if (this->ConfigThread && this->ConfigThread->IsDone()) {
            ...
            this->Logs->Log("CONFIG", LOG_DEBUG,
                            "Detected ConfigThread exiting, tidying up...");
