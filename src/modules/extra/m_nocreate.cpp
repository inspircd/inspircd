...
class NoCreate : public XLine {
  ...
  public:
    NoCreate(time_t _set_time, unsigned long _duration, const std::string& _source,
             const std::string& _reason, const std::string& _mask)
    ...
    bool MaskIsInsane(User* user, const std::string& mask) {
        ...
        for (user_hash::const_iterator u = users.begin(); u != users.end(); ++u) {
            if (InspIRCd::Match(u->second->nick, nick) &&
                    (InspIRCd::Match(u->second->MakeHost(), userhost, ascii_case_insensitive_map) ||
                     InspIRCd::MatchCIDR(u->second->MakeHostIP(), userhost,
        ...
        if (percent > itrigger) {
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "\2WARNING\2: %s tried to set a NoCreate mask of %s, which covers %.2f%% of the network!",
    ...
    CmdResult Handle(User* user, const Params& parameters) CXX11_OVERRIDE {
        ...
        if (target.find('!') == std::string::npos || target.find('@') == std::string::npos) {
            if (u && u->registered == REG_ALL) {
                ...
                user->WriteNotice(InspIRCd::Format("*** NoCreate: No user '%s' found",
                                                   target.c_str()));
                return CMD_FAILURE;
            }
        }
        ...
        if (parameters.size() >= 3) {
            if (this->MaskIsInsane(user, target)) {
                user->WriteNotice(InspIRCd::Format("*** NoCreate mask %s flagged as insane",
                                                   target.c_str()));
                return CMD_FAILURE;
            }
            ...
            NoCreate* nc = new NoCreate(ServerInstance->Time(), duration, user->nick,
                                        parameters[2], target);
            if (ServerInstance->XLines->AddLine(nc, user)) {
                if (!duration) {
                    ServerInstance->SNO->WriteToSnoMask('x',
                                                        "%s added permanent NoCreate for %s: %s",
                                                        user->nick.c_str(), target.c_str(), parameters[2].c_str());
                } else {
                    ServerInstance->SNO->WriteToSnoMask('x',
                                                        "%s added timed NoCreate for %s, expires in %s (on %s): %s",
                                                        user->nick.c_str(), target.c_str(), InspIRCd::DurationString(duration).c_str(),
                                                        ServerInstance->TimeString(ServerInstance->Time() + duration).c_str(),
                                                        parameters[2].c_str());
                }
            } else {
                delete nc;
                user->WriteNotice(InspIRCd::Format("*** NoCreate for %s already exists",
                                                   target.c_str()));
                return CMD_FAILURE;
            }
        }
        // Removing
            ...
            if (ServerInstance->XLines->DelLine(target.c_str(), "NOCREATE", reason, user)) {
                ...
                user->WriteNotice(
                    InspIRCd::Format("*** NoCreate %s not found in list, try /stats N",
                                     target.c_str()));
                return CMD_FAILURE;
            }
        }
...
class ModuleNoCreate : public Module, public Stats::EventListener {
  ...
  public:
    ...
    ModResult OnUserPreJoin(LocalUser* user, Channel* chan,
                            const std::string& cname, std::string&, const std::string&) CXX11_OVERRIDE {
        ...
        if (telluser) {
            user->WriteNumeric(ERR_BANNEDFROMCHAN, cname,
                               default_reason.empty() ? nc->reason.c_str() : default_reason.c_str());
        } else {
