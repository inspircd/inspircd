...
class LDAPBind : public LDAPRequest {
  ...
  public:
    LDAPBind(LDAPService* s, LDAPInterface* i, const std::string& w,
             const std::string& p)
...
class LDAPSearchRequest : public LDAPRequest {
  ...
  public:
    LDAPSearchRequest(LDAPService* s, LDAPInterface* i, const std::string& b,
                      int se, const std::string& f)
        : LDAPRequest(s, i, LDAP_SUCCESS)
        , base(b)
        , searchscope(se)
        , filter(f) {
...
class LDAPAdd : public LDAPRequest {
  ...
  public:
    LDAPAdd(LDAPService* s, LDAPInterface* i, const std::string& d,
            const LDAPMods& attr)
        : LDAPRequest(s, i, LDAP_SUCCESS)
        , dn(d)
        , attributes(attr) {
...
class LDAPModify : public LDAPRequest {
  ...
  public:
    LDAPModify(LDAPService* s, LDAPInterface* i, const std::string& b,
               const LDAPMods& attr)
        : LDAPRequest(s, i, LDAP_SUCCESS)
        , base(b)
        , attributes(attr) {
...
class LDAPCompare : public LDAPRequest {
  ...
  public:
    LDAPCompare(LDAPService* s, LDAPInterface* i, const std::string& d,
...
class LDAPService : public LDAPProvider, public SocketThread {
  private:
    ...
    int ldap_initialize(LDAP** ldap, const char* url) {
        ...
        if (up.field_set & (1 << UF_SCHEMA)) {
            const std::string schema(url, up.field_data[UF_SCHEMA].off,
        ...
        if (up.field_set & (1 << UF_PORT)) {
            const std::string portstr(url, up.field_data[UF_PORT].off,
                                      up.field_data[UF_PORT].len);
            port = ConvToNum<unsigned long>(portstr);
        }

        const std::string host(url, up.field_data[UF_HOST].off,
                               up.field_data[UF_HOST].len);
  ...
  public:
    ...
    void Bind(LDAPInterface* i, const std::string& who,
              const std::string& pass) CXX11_OVERRIDE {
        LDAPBind* b = new LDAPBind(this, i, who, pass);
        QueueRequest(b);
    }

    void Search(LDAPInterface* i, const std::string& base,
                const std::string& filter) CXX11_OVERRIDE {
        if (i == NULL) {
    ...
    void Add(LDAPInterface* i, const std::string& dn,
             LDAPMods& attributes) CXX11_OVERRIDE {
    ...
    void Modify(LDAPInterface* i, const std::string& base,
                LDAPMods& attributes) CXX11_OVERRIDE {
        LDAPModify* mod = new LDAPModify(this, i, base, attributes);
        QueueRequest(mod);
    }

    void Compare(LDAPInterface* i, const std::string& dn, const std::string& attr,
                 const std::string& val) CXX11_OVERRIDE {
  ...
  private:
    void BuildReply(int res, LDAPRequest* req) {
        ...
        if (res != req->success) {
            ldap_result->error = InspIRCd::Format("%s (%s)", ldap_err2string(res),
                                                  req->info().c_str());
            return;
        }
...
class ModuleLDAP : public Module {
  ...
  public:
    ...
    ~ModuleLDAP() {
        for (ServiceMap::iterator i = LDAPServices.begin(); i != LDAPServices.end();
                ++i) {
            LDAPService* conn = i->second;
            conn->join();
            conn->OnNotify();
...
int LDAPBind::run() {
    ...
    int i = ldap_sasl_bind_s(service->GetConnection(), LDAP_STR(who),
...
int LDAPSearchRequest::run() {
    return ldap_search_ext_s(service->GetConnection(), LDAP_STR(base), searchscope,
                             LDAP_STR(filter), NULL, 0, NULL, NULL, LDAP_TIME(tv), 0, &message);
}
...
int LDAPAdd::run() {
    ...
    int i = ldap_add_ext_s(service->GetConnection(), LDAP_STR(dn), mods, NULL,
                           NULL);
...
int LDAPModify::run() {
    LDAPMod** mods = LDAPService::BuildMods(attributes);
    int i = ldap_modify_ext_s(service->GetConnection(), LDAP_STR(base), mods, NULL,
                              NULL);
    LDAPService::FreeMods(mods);
    return i;
}
...
#ifdef _WIN32
    int ret = ldap_compare_ext_s(service->GetConnection(), LDAP_STR(dn),
                                 LDAP_STR(attr), NULL, &cred, NULL, NULL);
#else
    int ret = ldap_compare_ext_s(service->GetConnection(), dn.c_str(), attr.c_str(),
                                 &cred, NULL, NULL);
#endif
