...
class SQLConn : public SQL::Provider {
  ...
  public:
    SQLConn(Module* Parent, ConfigTag* tag)
        ...
        if (sqlite3_open_v2(host.c_str(), &conn, SQLITE_OPEN_READWRITE,
                            0) != SQLITE_OK) {
            // Even in case of an error conn must be closed
            sqlite3_close(conn);
            conn = NULL;
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "WARNING: Could not open DB with id: " + tag->getString("id"));
        }
    }
    ...
    void Submit(SQL::Query* query, const std::string& q,
