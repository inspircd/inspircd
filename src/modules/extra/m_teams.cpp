...
class ModuleTeams
  ...
  private:
    ...
    size_t ExecuteCommand(LocalUser* source, const char* cmd,
                          CommandBase::Params& parameters,
                          const std::string& team, size_t nickindex) {
        size_t targets = 0;
        std::string command(cmd);
        const user_hash& users = ServerInstance->Users->GetUsers();
        for (user_hash::const_iterator iter = users.begin(); iter != users.end();
                ++iter) {
    ...
    bool IsTeam(const std::string& param, std::string& team) {
        if (param.length() <= teamchar.length()
                || param.compare(0, teamchar.length(), teamchar) != 0) {
            return false;
        }
  ...
  public:
    ...
    ModResult OnCheckBan(User* user, Channel* channel,
