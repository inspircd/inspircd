...
typedef RAIIObj<mbedtls_entropy_context, mbedtls_entropy_init, mbedtls_entropy_free>
Entropy;

class CTRDRBG : private
    RAIIObj<mbedtls_ctr_drbg_context, mbedtls_ctr_drbg_init, mbedtls_ctr_drbg_free> {
  public:
    bool Seed(Entropy& entropy) {
        return (mbedtls_ctr_drbg_seed(get(), mbedtls_entropy_func, entropy.get(), NULL,
                                      0) == 0);
    }
...
class DHParams : public
    RAIIObj<mbedtls_dhm_context, mbedtls_dhm_init, mbedtls_dhm_free> {
  public:
    void set(const std::string& dhstr) {
        // Last parameter is buffer size, must include the terminating null
        int ret = mbedtls_dhm_parse_dhm(get(),
                                        reinterpret_cast<const unsigned char*>(dhstr.c_str()), dhstr.size()+1);
        ThrowOnError(ret, "Unable to import DH params");
    }
};

class X509Key : public
    RAIIObj<mbedtls_pk_context, mbedtls_pk_init, mbedtls_pk_free> {
  public:
    /** Import */
    X509Key(const std::string& keystr) {
#if MBEDTLS_VERSION_MAJOR >= 3
        int ret = mbedtls_pk_parse_key(get(),
...
class Curves {
  ...
  public:
    Curves(const std::string& str) {
        irc::sepstream ss(str, ':');
        for (std::string token; ss.GetToken(token); ) {
            const mbedtls_ecp_curve_info* curve = mbedtls_ecp_curve_info_from_name(
                    token.c_str());
            if (!curve) {
...
class X509CertList : public
    RAIIObj<mbedtls_x509_crt, mbedtls_x509_crt_init, mbedtls_x509_crt_free> {
  public:
    ...
    X509CertList(const std::string& certstr, bool allowempty = false) {
        ...
        int ret = mbedtls_x509_crt_parse(get(),
                                         reinterpret_cast<const unsigned char*>(certstr.c_str()), certstr.size()+1);
        ThrowOnError(ret, "Unable to load certificates");
    }

    bool empty() const {
        return (get()->raw.p != NULL);
    }
};

class X509CRL : public
    RAIIObj<mbedtls_x509_crl, mbedtls_x509_crl_init, mbedtls_x509_crl_free> {
  public:
    X509CRL(const std::string& crlstr) {
        if (crlstr.empty()) {
            return;
        }
        int ret = mbedtls_x509_crl_parse(get(),
                                         reinterpret_cast<const unsigned char*>(crlstr.c_str()), crlstr.size()+1);
...
#if MBEDTLS_VERSION_MAJOR >= 3
            if (mbedtls_pk_check_pair(&cert->pk, key.get(), mbedtls_ctr_drbg_random,
                                      0) == 0)
#else
...
class Context {
...
#ifdef INSPIRCD_MBEDTLS_LIBRARY_DEBUG
    static void DebugLogFunc(void* userptr, int level, const char* file, int line,
                             const char* msg) {
        // Remove trailing \n
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "%s:%d %.*s", file, line, len,
                                  msg);
    }
#endif
  ...
  public:
    ...
    const mbedtls_ssl_config* GetConf() const {
        return &conf;
    }
};
...
class mbedTLSIOHook : public SSLIOHook {
  private:
    ...
    int Handshake(StreamSocket* sock) {
        ...
        if (ret == 0) {
            ...
            SocketEngine::ChangeEventMask(sock,
    ...
    void VerifyCertificate() {
        ...
        certificate->fingerprint = GetProfile().GetHash().hash(cert->raw.p,
                                   cert->raw.len);
        ...
        certificate->invalid = ((flags & MBEDTLS_X509_BADCERT_BAD_KEY)
    ...
    static int Pull(void* userptr, unsigned char* buffer, size_t size) {
        ...
        const int ret = SocketEngine::Recv(sock, reinterpret_cast<char*>(buffer), size,
                                           0);
        if (ret < (int)size) {
  ...
  public:
    ...
    int OnStreamSocketWrite(StreamSocket* sock,
        ...
        while (!sendq.empty()) {
            ...
            int ret = mbedtls_ssl_write(&sess,
                                        reinterpret_cast<const unsigned char*>(buffer.data()), buffer.length());
    ...
    void ReadProfiles() {
        ...
        if (tags.first == tags.second) {
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "No <sslprofile> tags found; using settings from the deprecated <mbedtls> tag");
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "You have defined an <sslprofile> tag; you should use this in place of \"mbedtls\" when configuring TLS (SSL) connections in <bind:sslprofile> or <link:sslprofile>");
            for (ConfigIter i = tags.first; i != tags.second; ++i) {
                ConfigTag* tag = i->second;
                if (!stdalgo::string::equalsci(tag->getString("provider"), "mbedtls")) {
                    ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                              "Ignoring non-mbedTLS <sslprofile> tag at " + tag->getTagLocation());
                    continue;
                }
    ...
    void OnModuleRehash(User* user, const std::string &param) CXX11_OVERRIDE {
        ...
        try {
            ...
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "mbedTLS TLS (SSL) profiles have been reloaded.");
        } catch (ModuleException& ex) {
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "Failed to reload the mbedTLS TLS (SSL) profiles. " + ex.GetReason());
        }
    }
