  ...
  public:
    ...
    CmdResult HandleLocal(LocalUser* user,
        ...
        if (parameters[0].length() < minlength) {
            failrpl.Send(user, this, "NEED_MORE_CHARS", parameters[0], minlength,
                         "You must specify more characters to complete.");
            return CMD_FAILURE;
        }
        ...
        if (parameters.size() > 1) {
            ...
            if (!max || max > maxsuggestions) {
                failrpl.Send(user, this, "INVALID_MAX_SUGGESTIONS", parameters[1],
                             maxsuggestions, "The number of suggestions you requested was invalid.");
                return CMD_FAILURE;
            }
        }
