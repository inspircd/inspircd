...
public:
	...
	{
		List[I_OnUserDisconnect] = List[I_OnCheckReady] = List[I_OnRehash] = List[I_OnUserRegister] = 1;
	}
	...
	bool Connect()
	{
		...
		{
			if (verbose)
				ServerInstance->WriteOpers("LDAP connection failed: %s", ldap_err2string(res));
			conn = NULL;
			return false;			
		}
	...
	bool CheckCredentials(userrec* user)
	{
		...
		{
			if (verbose)
				ServerInstance->WriteOpers("Forbidden connection from %s!%s@%s (LDAP search returned more than one result: %s)", user->nick, user->ident, user->host, ldap_err2string(res));
			ldap_msgfree(msg);
			return false;
		}
		...
		{
			if (verbose)
				ServerInstance->WriteOpers("Forbidden connection from %s!%s@%s (LDAP search returned no results: %s)", user->nick, user->ident, user->host, ldap_err2string(res));
			ldap_msgfree(msg);
			return false;
		}
