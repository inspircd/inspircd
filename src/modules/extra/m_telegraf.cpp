...
struct TelegrafLine {
    ...
    std::string format() {
        ...
        bool first = true;
        for (std::map<std::string, std::string>::const_iterator i = fields.begin();
                i != fields.end(); i++) {
            if (first) {
                out += " ";
                first = false;
  ...
  public:
    TelegrafCommand(Module *parent)
        : Command(parent, "TELEGRAF", 1),
          actions(cmd_actions, cmd_actions + sizeof(cmd_actions) / sizeof(
                      cmd_actions[0])) {
...
class TelegrafModule : public Module {
  ...
  public:
    ...
    void StartMetrics(bool restarted = false) {
        ...
        if (!silent) {
            ServerInstance->SNO->WriteGlobalSno('a', "METRICS: Telegraf metrics %sstarted.",
                                                restarted ? "re" : "");
        }
    }
...
CmdResult TelegrafCommand::Handle(User* user,
                                  const CommandBase::Params& parameters) {
    if (actions.find(parameters[0]) == actions.end()) {
        if (IS_LOCAL(user)) {
            user->WriteNumeric(RPL_SYNTAX, "%s :SYNTAX %s %s", user->nick.c_str(),
    ...
    if (parameters.size() > 1
            && !InspIRCd::Match(ServerInstance->Config->ServerName, parameters[1])) {
        // Route the command only to the remote server specified
        return CMD_SUCCESS;
    }

    TelegrafModule *mod = static_cast<TelegrafModule *> (static_cast<Module *>
                          (creator));
    ...
    if (parameters[0] == "start") {
        ...
        if (mod->tSock) {
            ...
            for (std::map<std::string, std::string>::const_iterator i = line.tags.begin();
                    i != line.tags.end(); ++i) {
    ...
    for (std::vector<std::string>::size_type i = 0; i < messages.size(); ++i) {
        if (parameters.size() > 1) {
            user->WriteNotice(InspIRCd::Format("*** From %s: %s",
                                               ServerInstance->Config->ServerName.c_str(), messages[i].c_str()));
        } else {
...
void TelegrafSocket::SendMetrics() {
    ...
    WriteData(out);
    ServerInstance->Logs->Log("TELEGRAF", LOG_DEBUG, "Sent Telegraf metrics: %s",
                              out.c_str());
}
...
TelegrafLine TelegrafSocket::GetMetrics() {
    ...
    line.fields["main_loop_time"] = ConvToStr(
                                        creator->metrics.getAverageLoopTime());
    return line;
}
