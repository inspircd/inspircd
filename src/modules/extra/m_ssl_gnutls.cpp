...
# endif
    ...
    gnutls_digest_algorithm_t get() const {
        return hash;
    }
};
...
class DHParams {
    ...
    DHParams() {
        ThrowOnError(gnutls_dh_params_init(&dh_params),
  ...
  public:
    ...
    static SMART_PTR<DHParams> Import(const std::string& dhstr) {
        ...
        int ret = gnutls_dh_params_import_pkcs3(dh->dh_params, Datum(dhstr).get(),
                                                GNUTLS_X509_FMT_PEM);
...
class X509Key {
    ...
    class RAIIKey {
      public:
        ...
        RAIIKey() {
            ThrowOnError(gnutls_x509_privkey_init(&key),
...
class X509CRL : public refcountbase {
  ...
  public:
    ...
    X509CRL(const std::string& crlstr) {
        int ret = gnutls_x509_crl_import(get(), Datum(crlstr).get(),
                                         GNUTLS_X509_FMT_PEM);
  ...
  public:
    Priority(const std::string& priorities) {
        ...
        if (ret < 0) {
            ...
            throw Exception("Unable to initialize priorities to \"" + priorities + "\": " +
                            gnutls_strerror(ret) + " Syntax error at position " + ConvToStr((unsigned int) (
                                        prioerror - priocstr)));
        }
    }
        ...
        for (std::string token; ss.GetToken(token); ) {
            ...
            if (gnutls_priority_init(&test, ret.c_str(), NULL) < 0) {
                ...
                ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                          "Priority string token not recognized: \"%s\"", token.c_str());
                ret.erase(prevpos);
            } else {
                // Worked
...
class CertCredentials {
  ...
  public:
    CertCredentials() {
        ThrowOnError(gnutls_certificate_allocate_credentials(&cred),
                     "Cannot allocate certificate credentials");
    }
...
class X509Credentials : public CertCredentials {
  ...
  public:
    ...
    void SetCA(SMART_PTR<X509CertList>& certlist, SMART_PTR<X509CRL>& CRL) {
        ...
        if (certlist.get()) {
            int ret = gnutls_certificate_set_x509_trust(cred, certlist->raw(),
                      certlist->size());
...
class DataReader {
  ...
  public:
    DataReader(gnutls_session_t sess)
        ...
        retval = gnutls_record_recv(sess, buffer,
...
class Profile {
    ...
    static std::string GetPrioStr(const std::string& profilename, ConfigTag* tag) {
        ...
        if (!tag->getBool("strictpriority", found)) {
            ...
            if (stripped.empty()) {
                ...
                stripped = GnuTLS::Priority::RemoveUnknownTokens(
                               GnuTLS::Priority::GetDefault());
                ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                          "Priority string for profile \"%s\" contains unknown tokens and stripping it didn't yield a working one either, falling back to \"%s\"",
                                          profilename.c_str(), stripped.c_str());
            } else if ((found) && (stripped != priostr)) {
                // Prio string was set in the config and we ended up with something that works but different
                ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                          "Priority string for profile \"%s\" contains unknown tokens, stripped to \"%s\"",
                                          profilename.c_str(), stripped.c_str());
            }
  ...
  public:
    ...
    X509Credentials& GetX509Credentials() {
        return x509cred;
    }
    gnutls_digest_algorithm_t GetHash() const {
        return hash.get();
    }
...
class GnuTLSIOHook : public SSLIOHook {
  private:
    ...
    void VerifyCertificate() {
        ...
        const gnutls_datum_t* cert_list = gnutls_certificate_get_peers(this->sess,
                                          &cert_list_size);
        if (cert_list == NULL) {
        ...
        buffer_size = sizeof(buffer);
        if ((ret = gnutls_x509_crt_get_fingerprint(cert, GetProfile().GetHash(), buffer,
                   &buffer_size)) < 0) {
            certinfo->error = gnutls_strerror(ret);
        } else {
         ...
         */
        if ((gnutls_x509_crt_get_expiration_time(cert) < ServerInstance->Time())
                || (gnutls_x509_crt_get_activation_time(cert) > ServerInstance->Time())) {
...
info_done_dealloc:
    ...
    static void ProcessDNString(const char* buffer, size_t buffer_size,
                                std::string& out) {
    ...
    static ssize_t gnutls_pull_wrapper(gnutls_transport_ptr_t session_wrap,
        ...
        if (rv < 0) {
             ...
             */
            gnutls_transport_set_errno(session->sess,
                                       SocketEngine::IgnoreError() ? EAGAIN : errno);
        }
#endif
    ...
    static ssize_t VectorPush(gnutls_transport_ptr_t transportptr,
        ...
        int ret = SocketEngine::WriteV(sock, reinterpret_cast<const iovec*>(iov),
                                       iovcnt);
#ifdef _WIN32
    ...
    static ssize_t gnutls_push_wrapper(gnutls_transport_ptr_t session_wrap,
        ...
        int rv = SocketEngine::Send(sock, reinterpret_cast<const char *>(buffer), size,
                                    0);
  ...
  public:
    GnuTLSIOHook(IOHookProvider* hookprov, StreamSocket* sock,
                 inspircd_gnutls_session_init_flags_t flags)
        : SSLIOHook(hookprov)
        , sess(NULL)
#ifdef INSPIRCD_GNUTLS_HAS_CORK
        , gbuffersize(0)
    ...
    int OnStreamSocketWrite(StreamSocket* user,
        ...
        while (true) {
            ...
            while ((!sendq.empty())
                    && (gbuffersize < GetProfile().GetOutgoingRecordSize())) {
        ...
        while (!sendq.empty()) {
            ...
            ret = HandleWriteRet(user, gnutls_record_send(this->sess, buffer.data(),
                                 buffer.length()));
    ...
    void GetCiphersuite(std::string& out) const CXX11_OVERRIDE {
        ...
        out.append(UnknownIfNULL(gnutls_protocol_get_name(gnutls_protocol_get_version(
                                     sess)))).push_back('-');
        out.append(UnknownIfNULL(gnutls_kx_get_name(gnutls_kx_get(
                                     sess)))).push_back('-');
        out.append(UnknownIfNULL(gnutls_cipher_get_name(gnutls_cipher_get(
                                     sess)))).push_back('-');
    ...
    bool GetServerName(std::string& out) const CXX11_OVERRIDE {
        ...
        if (gnutls_server_name_get(sess, &nameBuffer[0], &nameLength, &nameType,
                                   0) != GNUTLS_E_SHORT_MEMORY_BUFFER) {
            return false;
        }
...
int GnuTLS::X509Credentials::cert_callback(gnutls_session_t sess,
        const gnutls_datum_t* req_ca_rdn, int nreqs,
        const gnutls_pk_algorithm_t* sign_algos, int sign_algos_length,
        cert_cb_last_param_type* st) {
...
#endif
    StreamSocket* sock = reinterpret_cast<StreamSocket*>(gnutls_transport_get_ptr(
                             sess));
    GnuTLS::X509Credentials& cred = static_cast<GnuTLSIOHook*>(sock->GetModHook(
                                        thismod))->GetProfile().GetX509Credentials();
...
class ModuleSSLGnuTLS : public Module {
    ...
    void ReadProfiles() {
        ...
        if (tags.first == tags.second) {
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "No <sslprofile> tags found; using settings from the deprecated <gnutls> tag");
            ...
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "You have defined an <sslprofile> tag; you should use this in place of \"gnutls\" when configuring TLS (SSL) connections in <bind:sslprofile> or <link:sslprofile>");
            for (ConfigIter i = tags.first; i != tags.second; ++i) {
                ConfigTag* tag = i->second;
                if (!stdalgo::string::equalsci(tag->getString("provider"), "gnutls")) {
                    ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                              "Ignoring non-GnuTLS <sslprofile> tag at " + tag->getTagLocation());
                    continue;
                }
    ...
    void OnModuleRehash(User* user, const std::string &param) CXX11_OVERRIDE {
        ...
        try {
            ...
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "GnuTLS TLS (SSL) profiles have been reloaded.");
        } catch (ModuleException& ex) {
            ServerInstance->SNO->WriteToSnoMask('a',
                                                "Failed to reload the GnuTLS TLS (SSL) profiles. " + ex.GetReason());
        }
    }
