 ...
 public:
	...
	CmdResult Handle (const std::vector<std::string>& parameters, User *user)
	{
		...
		if (target && IS_LOCAL(target))
		{
			...
			{
				SnomaskManager::Write(SNO_REMOTE | SNO_BROADCAST, SnomaskManager::announcement, oldnick+" used SANICK to change "+newnick+" to "+parameters[1]);
			}
			else
			{
				SnomaskManager::Write(SNO_REMOTE | SNO_BROADCAST, SnomaskManager::announcement, oldnick+" failed SANICK (from "+newnick+" to "+parameters[1]+")");
			}
		}
