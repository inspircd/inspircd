		...
		for (UCListIter i = user->chans.begin(); i != user->chans.end(); i++)
		{
			...
			if (f)
			{
				...
				if (f->shouldlock())
				{
					f->clear();
					f->lock();
					channel->WriteChannelWithServ(ServerInstance->Config->ServerName.c_str(), "NOTICE %s :No nick changes are allowed for 60 seconds because there have been more than %d nick changes in %d seconds.", channel->name.c_str(), f->nicks, f->secs);
					return MOD_RES_DENY;
				}
			}
		}
