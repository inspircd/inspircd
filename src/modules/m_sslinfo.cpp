...
class CommandSSLInfo : public SplitCommand {
  private:
    ...
    void HandleUserInternal(LocalUser* source, User* target, bool verbose) {
        ...
        if (!cert) {
            source->WriteNotice(InspIRCd::Format("*** %s is not connected using TLS (SSL).",
                                                 target->nick.c_str()));
        } else if (cert->GetError().length()) {
            source->WriteNotice(
                InspIRCd::Format("*** %s is connected using TLS (SSL) but has not specified a valid client certificate (%s).",
                                 target->nick.c_str(), cert->GetError().c_str()));
        } else if (!verbose) {
            source->WriteNotice(
                InspIRCd::Format("*** %s is connected using TLS (SSL) with a valid client certificate (%s).",
    ...
    CmdResult HandleUser(LocalUser* source, const std::string& nick) {
        ...
        if (operonlyfp && !source->IsOper() && source != target) {
            source->WriteNumeric(ERR_NOPRIVILEGES,
                                 "You must be a server operator to view TLS (SSL) client certificate information for other users.");
            return CMD_FAILURE;
        }
    ...
    CmdResult HandleChannel(LocalUser* source, const std::string& channel) {
        ...
        if (operonlyfp && !source->IsOper()) {
            source->WriteNumeric(ERR_NOPRIVILEGES,
                                 "You must be a server operator to view TLS (SSL) client certificate information for channels.");
            return CMD_FAILURE;
        }

        if (!source->IsOper() && chan->GetPrefixValue(source) < OP_VALUE) {
            source->WriteNumeric(Numerics::ChannelPrivilegesNeeded(chan, OP_VALUE,
                                 "view TLS (SSL) client certificate information"));
            return CMD_FAILURE;
        }

        if (sslonlymode) {
            source->WriteNotice(
                InspIRCd::Format("*** %s %s have channel mode +%c (%s) set.",
  ...
  public:
    ...
    void OnWhois(Whois::Context& whois) CXX11_OVERRIDE {
        ...
        if (cert) {
            ...
            if ((!cmd.operonlyfp || whois.IsSelfWhois() || whois.GetSource()->IsOper())
                    && !cert->fingerprint.empty()) {
                whois.SendLine(RPL_WHOISCERTFP,
                               InspIRCd::Format("has TLS (SSL) client certificate fingerprint %s",
            ...
            if (i != ServerInstance->Config->oper_blocks.end()) {
                ...
                if (ifo->oper_block->getBool("sslonly") && !cert) {
                    ...
                    ServerInstance->SNO->WriteGlobalSno('o',
                                                        "WARNING! Failed oper attempt by %s using login '%s': a secure connection is required.",
                                                        user->GetFullRealHost().c_str(), parameters[0].c_str());
                    return MOD_RES_DENY;
                }

                std::string fingerprint;
                if (ifo->oper_block->readString("fingerprint", fingerprint) && (!cert
                        || !MatchFP(cert, fingerprint))) {
                    user->WriteNumeric(ERR_NOOPERHOST, "Invalid oper credentials");
                    user->CommandFloodPenalty += 10000;
                    ServerInstance->SNO->WriteGlobalSno('o',
                                                        "WARNING! Failed oper attempt by %s using login '%s': their TLS (SSL) client certificate fingerprint does not match.",
                                                        user->GetFullRealHost().c_str(), parameters[0].c_str());
                    return MOD_RES_DENY;
                }
            }
    ...
    void OnPostConnect(User* user) CXX11_OVERRIDE {
        ...
        if (cert && !cert->GetFingerprint().empty()) {
            text.append(" and your TLS (SSL) client certificate fingerprint is ").append(
                cert->GetFingerprint());
        }
        ...
        for (ServerConfig::OperIndex::const_iterator i = ServerInstance->Config->oper_blocks.begin(); i != ServerInstance->Config->oper_blocks.end(); ++i) {
            ...
            if (stdalgo::string::equalsci(autologin, "if-host-match")) {
                ...
                do_login = InspIRCd::MatchMask(ifo->oper_block->getString("host"), userHost,
                                               userIP);
    ...
    ModResult OnSetConnectClass(LocalUser* user,
        ...
        if (error) {
            ServerInstance->Logs->Log("CONNECTCLASS", LOG_DEBUG,
                                      "The %s connect class is not suitable as it requires %s",
