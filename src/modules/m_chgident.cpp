	...
	CmdResult Handle(const std::vector<std::string> &parameters, User *user)
	{
		...
		if (IS_LOCAL(dest))
		{
			...
			if (!user->server->IsULine())
				SnomaskManager::Write(SNO_REMOTE | SNO_BROADCAST, SnomaskManager::announcement, "%s used CHGIDENT to change %s's ident to '%s'", user->nick.c_str(), dest->nick.c_str(), dest->ident.c_str());
		}
