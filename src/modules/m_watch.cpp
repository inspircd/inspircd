...
class CommandWatch : public SplitCommand {
    ...
    static void SendOnlineOffline(LocalUser* user, const std::string& nick,
        ...
        if (target) {
            ...
            if (target->IsAway()) {
                user->WriteNumeric(RPL_NOWISAWAY, target->nick, target->ident,
                                   target->GetDisplayedHost(), (unsigned long)target->awaytime, "is away");
            } else {
                user->WriteNumeric(RPL_NOWON, target->nick, target->ident,
                                   target->GetDisplayedHost(), (unsigned long)target->age, "is online");
            }
    ...
    void HandleMinus(LocalUser* user, const std::string& nick) {
        ...
        User* target = IRCv3::Monitor::Manager::FindNick(nick);
        if (target) {
            user->WriteNumeric(RPL_WATCHOFF, target->nick, target->ident,
                               target->GetDisplayedHost(), (unsigned long)target->age, "stopped watching");
        } else {
    ...
    void HandleStats(LocalUser* user) {
        ...
        const IRCv3::Monitor::WatchedList& list = manager.GetWatched(user);
        user->WriteNumeric(RPL_WATCHSTAT,
                           InspIRCd::Format("You have %lu and are on 0 WATCH entries",
...
class ModuleWatch
    ...
    void SendAlert(User* user, const std::string& nick, unsigned int numeric,
                   const char* numerictext, time_t shownts) {
        ...
        num.push(nick).push(user->ident).push(user->GetDisplayedHost()).push(ConvToStr(
                    shownts)).push(numerictext);
