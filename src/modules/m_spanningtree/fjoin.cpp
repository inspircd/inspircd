...
CmdResult CommandFJoin::Handle(User* srcuser, Params& params) {
    ...
    if (!chan) {
        ...
        if (TS != ourTS) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                      "Merge FJOIN received for %s, ourTS: %lu, TS: %lu, difference: %ld",
                                      chan->name.c_str(), (unsigned long)ourTS, (unsigned long)TS,
                                      (long)(ourTS - TS));
            ...
            if (ourTS < TS) {
                ...
                if (!sourceserver->IsBursting()) {
                    ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                              "Server %s recreated channel %s with higher TS, resyncing",
                                              sourceserver->GetName().c_str(), chan->name.c_str());
    ...
    if (apply_other_sides_modes) {
        ServerInstance->Modes.ModeParamsToChangeList(srcuser, MODETYPE_CHANNEL, params,
                modechangelist, 2, params.size() - 1);
        ServerInstance->Modes->Process(srcuser, chan, NULL, modechangelist,
                                       ModeParser::MODE_LOCALONLY | ModeParser::MODE_MERGE);
    ...
    std::string item;
    Modes::ChangeList* modechangelistptr = (apply_other_sides_modes ?
                                            &modechangelist : NULL);
...
void CommandFJoin::ProcessModeUUIDPair(const std::string& item,
                                       TreeServer* sourceserver, Channel* chan, Modes::ChangeList* modechangelist,
                                       FwdFJoinBuilder& fwdfjoin) {
    std::string::size_type comma = item.find(',');

    // Comma not required anymore if the user has no modes
    const std::string::size_type ubegin = (comma == std::string::npos ? 0 : comma
                                           +1);
    ...
    std::string::const_iterator modeendit =
        item.begin(); // End of the "ov" mode string
...
void CommandFJoin::RemoveStatus(Channel* c) {
    ...
    ServerInstance->Modes->Process(ServerInstance->FakeClient, c, NULL, changelist,
                                   ModeParser::MODE_LOCALONLY);
}

void CommandFJoin::LowerTS(Channel* chan, time_t TS,
                           const std::string& newname) {
    if (Utils->AnnounceTSChange) {
        // WriteRemoteNotice is not used here because the message only needs to go to the local server.
        chan->WriteNotice(InspIRCd::Format("Creation time of %s changed from %s to %s",
                                           newname.c_str(),
...
void CommandFJoin::Builder::add(Membership* memb,
                                std::string::const_iterator mbegin, std::string::const_iterator mend) {
...
bool CommandFJoin::Builder::has_room(std::string::size_type nummodes) const {
    return ((str().size() + nummodes + UIDGenerator::UUID_LENGTH + 2 +
             membid_max_digits + 1) <= maxline);
}
...
void FwdFJoinBuilder::add(Membership* memb, std::string::const_iterator mbegin,
                          std::string::const_iterator mend) {
