...
CmdResult CommandRConnect::Handle(User* user, const Params& parameters) {
    ...
    if (!Utils->FindServerMask(parameters[0])) {
        user->WriteRemoteNotice(
            InspIRCd::Format("*** RCONNECT: Server \002%s\002 isn't connected to the network!",
                             parameters[0].c_str()));
        return CMD_FAILURE;
    }
    ...
    if (InspIRCd::Match(ServerInstance->Config->ServerName,parameters[0])) {
        ...
        ServerInstance->SNO->WriteToSnoMask('l',
                                            "Remote CONNECT from %s matching \002%s\002, connecting server \002%s\002",
         ...
         */
        if (IS_LOCAL(user)) {
            user->WriteNotice("*** RCONNECT: Sending remote connect to \002 " +
                              parameters[0] + "\002 to connect server \002" + parameters[1] + "\002.");
        }
    }
    return CMD_SUCCESS;
}

RouteDescriptor CommandRConnect::GetRouting(User* user,
