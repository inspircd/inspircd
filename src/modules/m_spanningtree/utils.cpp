...
ModResult ModuleSpanningTree::OnAcceptConnection(int newsock,
        ListenSocket* from, irc::sockets::sockaddrs* client,
    ...
    for (std::vector<std::string>::iterator i = Utils->ValidIPs.begin();
            i != Utils->ValidIPs.end(); i++) {
        if (*i == "*" || *i == incomingip
                || irc::sockets::cidr_mask(*i).match(*client)) {
    ...
    ServerInstance->SNO->WriteToSnoMask('l',
                                        "Server connection from %s denied (no link blocks with that IP address)",
                                        incomingip.c_str());
    return MOD_RES_DENY;
}
 ...
 */
TreeServer* SpanningTreeUtilities::FindServerMask(const std::string
        &ServerName) {
...
SpanningTreeUtilities::SpanningTreeUtilities(ModuleSpanningTree* C)
    ...
    , PingFreq(
          60) { // XXX: TreeServer constructor reads this and TreeRoot is created before the config is read, so init it to something (value doesn't matter) to avoid a valgrind warning in TimerManager on unload
...
CullResult SpanningTreeUtilities::cull() {
    ...
    for(TimeoutList::iterator i = timeoutlist.begin(); i != timeoutlist.end();
            ++i) {
...
void SpanningTreeUtilities::GetListOfServersForChannel(Channel* c,
        TreeSocketSet& list, char status, const CUList& exempt_list) {
    ...
    for (Channel::MemberMap::const_iterator i = ulist.begin(); i != ulist.end();
        ...
        if (exempt_list.find(i->first) == exempt_list.end()) {
            ...
            TreeServer::ChildServers::iterator citer = std::find(children.begin(),
                    children.end(), best);
    ...
    for (TreeServer::ChildServers::const_iterator i = children.begin();
            i != children.end(); ++i) {
        ModResult result;
        FIRST_MOD_RESULT_CUSTOM(Creator->GetBroadcastEventProvider(),
                                ServerProtocol::BroadcastEventListener, OnBroadcastMessage, result, (c, *i));
...
void SpanningTreeUtilities::DoOneToAllButSender(const CmdBuilder& params,
        TreeServer* omitroute) {
...
void SpanningTreeUtilities::DoOneToOne(const CmdBuilder& params,
                                       Server* server) {
...
void SpanningTreeUtilities::RefreshIPCache() {
    ValidIPs.clear();
    for (std::vector<reference<Link> >::iterator i = LinkBlocks.begin();
            i != LinkBlocks.end(); ++i) {
        Link* L = *i;
        bool isunix = L->IPAddr.find('/') != std::string::npos;
        if (!L->Port && !isunix) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Ignoring a link block without a port.");
            /* Invalid link block */
            continue;
        }
        ...
        if ((L->IPAddr == "*") || (isunix) || (ipvalid)) {
            ...
            SecurityIPResolver* sr = new SecurityIPResolver(Creator, *this->Creator->DNS,
                    L->IPAddr, L, DNS::QUERY_AAAA);
            try {
...
void SpanningTreeUtilities::ReadConfiguration() {
    ...
    quiet_bursts =
        ServerInstance->Config->ConfValue("performance")->getBool("quietbursts");
    ...
    for(ConfigIter i = tags.first; i != tags.second; ++i) {
        ...
        if (L->Name.empty()) {
            throw ModuleException("Invalid configuration, found a link tag without a name!"
                                  + (!L->IPAddr.empty() ? " IP address: "+L->IPAddr : ""));
        }

        if (L->Name.find('.') == std::string::npos) {
            throw ModuleException("The link name '"+L->Name
                                  +"' is invalid as it must contain at least one '.' character");
        }

        if (L->Name.length() > ServerInstance->Config->Limits.MaxHost) {
            throw ModuleException("The link name '"+L->Name
                                  +"' is invalid as it is longer than " + ConvToStr(
                                      ServerInstance->Config->Limits.MaxHost) + " characters");
        }

        if (L->RecvPass.empty()) {
            throw ModuleException("Invalid configuration for server '"+L->Name
                                  +"', recvpass not defined");
        }

        if (L->SendPass.empty()) {
            throw ModuleException("Invalid configuration for server '"+L->Name
                                  +"', sendpass not defined");
        }

        if ((L->SendPass.find(' ') != std::string::npos)
                || (L->RecvPass.find(' ') != std::string::npos)) {
            throw ModuleException("Link block '" + L->Name +
                                  "' has a password set that contains a space character which is invalid");
        }

        if ((L->SendPass[0] == ':') || (L->RecvPass[0] == ':')) {
            throw ModuleException("Link block '" + L->Name +
                                  "' has a password set that begins with a colon (:) which is invalid");
        }

        if (L->IPAddr.empty()) {
            L->IPAddr = "*";
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Configuration warning: Link block '" + L->Name +
                                      "' has no IP defined! This will allow any IP to connect as this server, and MAY not be what you want.");
        }

        if (!L->Port && L->IPAddr.find('/') == std::string::npos) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEFAULT,
                                      "Configuration warning: Link block '" + L->Name +
                                      "' has no port defined, you will not be able to /connect it.");
        }

        L->Fingerprint.erase(std::remove(L->Fingerprint.begin(), L->Fingerprint.end(),
                                         ':'), L->Fingerprint.end());
    ...
    for (server_hash::const_iterator i = serverlist.begin(); i != serverlist.end();
            ++i) {
        i->second->CheckULine();
    }
...
void SpanningTreeUtilities::SendChannelMessage(User* source, Channel* target,
        const std::string& text, char status, const ClientProtocol::TagMap& tags,
        const CUList& exempt_list, const char* message_type, TreeSocket* omit) {
