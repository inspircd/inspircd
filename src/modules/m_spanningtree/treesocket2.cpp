...
void TreeSocket::Split(const std::string& line, std::string& tags,
                       std::string& prefix, std::string& command, CommandBase::Params& params) {
    std::string token;
...
void TreeSocket::ProcessLine(std::string &line) {
    ...
    ServerInstance->Logs->Log(MODNAME, LOG_RAWIO, "S[%d] I %s", this->GetFd(),
                              line.c_str());
            ...
            if (!params.empty()) {
                ...
                if ((delta < -60) || (delta > 60)) {
                    ServerInstance->SNO->WriteGlobalSno('l',
                                                        "\002ERROR\002: Your clocks are off by %ld seconds (this is more than one minute). Link aborted, \002PLEASE SYNC YOUR CLOCKS!\002",
                                                        labs((long)delta));
                    SendError("Your clocks are out by "+ConvToStr(labs((long)delta))
                              +" seconds (this is more than one minute). Link aborted, PLEASE SYNC YOUR CLOCKS!");
                    return;
                } else if ((delta < -15) || (delta > 15)) {
                    ServerInstance->SNO->WriteGlobalSno('l',
                                                        "\002WARNING\002: Your clocks are off by %ld seconds. Please consider syncing your clocks.",
                                                        labs((long)delta));
                }
            }
...
User* TreeSocket::FindSource(const std::string& prefix,
    ...
    if ((prefix.length() == UIDGenerator::UUID_LENGTH) && (isdigit(prefix[0])) &&
            ((command == "FMODE") || (command == "MODE") || (command == "KICK")
             || (command == "TOPIC") || (command == "KILL") || (command == "ADDLINE")
             || (command == "DELLINE"))) {
...
void TreeSocket::ProcessTag(User* source, const std::string& tag,
    ...
    const Events::ModuleEventProvider::SubscriberList& list =
        Utils->Creator->tagevprov.GetSubscribers();
    for (Events::ModuleEventProvider::SubscriberList::const_iterator i =
        ...
        if (res == MOD_RES_ALLOW) {
            tags.insert(std::make_pair(tagkey, ClientProtocol::MessageTagData(tagprov,
                                       tagval)));
...
void TreeSocket::ProcessConnectedLine(std::string& taglist, std::string& prefix,
                                      std::string& command, CommandBase::Params& params) {
    User* who = FindSource(prefix, command);
    if (!who) {
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "Command '%s' from unknown prefix '%s'! Dropping entire command.",
    ...
    if (server->GetSocket() != this) {
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                  "Protocol violation: Fake direction '%s' from connection '%s'", prefix.c_str(),
                                  linkID.c_str());
        return;
    }
    ...
    if ((!params.empty()) && (params.back().empty())
            && (!cmdbase->allow_empty_last_param)) {
...
void TreeSocket::OnTimeout() {
    ServerInstance->SNO->WriteGlobalSno('l',
                                        "CONNECT: Connection to \002%s\002 timed out.", linkID.c_str());
}
...
void TreeSocket::Close() {
    ...
    if (MyRoot && !MyRoot->IsDead()) {
        ...
        ServerInstance->SNO->WriteGlobalSno('l', "Connection to '\002%s\002' failed.",
                                            linkID.c_str());
    }
    ...
    if (server_uptime) {
        ...
        ServerInstance->SNO->WriteGlobalSno('l',
                                            "Connection to '\002%s\002' was established for %s", linkID.c_str(),
                                            timestr.c_str());
    }
}

void TreeSocket::FinishAuth(const std::string& remotename,
                            const std::string& remotesid, const std::string& remotedesc, bool hidden) {
    this->LinkState = CONNECTED;
    Utils->timeoutlist.erase(this);

    linkID = remotename;

    MyRoot = new TreeServer(remotename, remotedesc, remotesid, Utils->TreeRoot,
                            this, hidden);
