...
CmdResult CommandAddLine::Handle(User* usr, std::vector<std::string>& params)
{
	...
	if (!xlf)
	{
		SnomaskManager::Write(SNO_LOCAL, SnomaskManager::debug,"%s sent me an unknown ADDLINE type (%s).",setter.c_str(),params[0].c_str());
		return CMD_FAILURE;
	}
	...
	{
		SnomaskManager::Write(SNO_LOCAL, SnomaskManager::debug,"Unable to ADDLINE type %s from %s: %s", params[0].c_str(), setter.c_str(), e.GetReason().c_str());
		return CMD_FAILURE;
	}
	...
	if (ServerInstance->XLines->AddLine(xl, NULL))
	{
		if (xl->duration)
		{
			...
			SnomaskManager::Write(SNO_REMOTE, SnomaskManager::xline,"%s added %s%s on %s to expire on %s: %s",setter.c_str(),params[0].c_str(),params[0].length() == 1 ? "-line" : "",
					params[1].c_str(), timestr.c_str(), params[5].c_str());
		}
		else
		{
			SnomaskManager::Write(SNO_REMOTE, SnomaskManager::xline,"%s added permanent %s%s on %s: %s",setter.c_str(),params[0].c_str(),params[0].length() == 1 ? "-line" : "",
