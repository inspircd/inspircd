...
CmdResult CommandFMode::Handle(User* who, std::vector<std::string>& params)
{
	...
	if (!TS)
	{
		...
		SnomaskManager::Write(SNO_LOCAL, SnomaskManager::debug, "WARNING: The server %s is sending FMODE with a TS of zero. Total craq, dropping link.", who->server->GetName().c_str());
		return CMD_INVALID;
	}
