...
static const char proxy_signature[13] =
...
class HAProxyHook : public IOHookMiddle {
  private:
    ...
    size_t ReadProxyTLV(StreamSocket* sock, size_t start_index,
    ...
    bool ReadProxyTLVSSL(StreamSocket* sock, size_t start_index,
                         uint16_t buffer_length) {
    ...
    int ReadProxyHeader(StreamSocket* sock, std::string& destrecvq) {
        ...
        command = static_cast<HAProxyCommand>(header.version_command &
                                              PP2_COMMAND_MASK);
        switch (command) {
        case HPC_LOCAL:
...
void HAProxyHookProvider::OnAccept(StreamSocket* sock,
