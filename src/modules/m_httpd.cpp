...
class HttpServerSocket : public BufferedSocket, public Timer,
  ...
  private:
    ...
    bool Tick(time_t currtime) CXX11_OVERRIDE {
        if (!messagecomplete) {
            ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "HTTP socket %d timed out",
                                      GetFd());
            Close();
            return false;
        }
    ...
    static void ConfigureParser() {
        ...
        parser_settings.on_header_field =
            DataCallback<&HttpServerSocket::OnHeaderField>;
        parser_settings.on_header_value =
            DataCallback<&HttpServerSocket::OnHeaderValue>;
        parser_settings.on_headers_complete =
            Callback<&HttpServerSocket::OnHeadersComplete>;
        parser_settings.on_body = DataCallback<&HttpServerSocket::OnBody>;
        parser_settings.on_message_complete =
            Callback<&HttpServerSocket::OnMessageComplete>;
    }
  ...
  public:
    HttpServerSocket(int newfd, const std::string& IP, ListenSocket* via,
        ...
        if ((!via->iohookprovs.empty()) && (via->iohookprovs.back())) {
            ...
            if (!getError().empty()) {
                ServerInstance->Logs->Log(MODNAME, LOG_DEBUG,
                                          "HTTP socket %d encountered a hook error: %s",
    ...
    void SendHTTPError(unsigned int response, const char* errstr = NULL) {
        ...
        ServerInstance->Logs->Log(MODNAME, LOG_DEBUG, "Sending HTTP error %u: %s",
                                  response, errstr);
    ...
    void SendHeaders(unsigned long size, unsigned int response,
                     HTTPHeaders &rheaders) {
        WriteData(InspIRCd::Format("HTTP/%u.%u %u %s\r\n",
                                   parser.http_major ? parser.http_major : 1,
                                   parser.http_major ? parser.http_minor : 1, response,
                                   http_status_str((http_status)response)));
    ...
    void ServeData() {
        ...
        FIRST_MOD_RESULT_CUSTOM(*aclevprov, HTTPACLEventListener, OnHTTPACLCheck,
                                MOD_RESULT, (acl));
        if (MOD_RESULT != MOD_RES_DENY) {
            HTTPRequest request(method, parsed, &headers, this, ip, body);
            FIRST_MOD_RESULT_CUSTOM(*reqevprov, HTTPRequestEventListener, OnHTTPRequest,
                                    MOD_RESULT, (request));
    ...
    bool ParseURI(const std::string& uristr, HTTPRequestURI& out) {
        ...
        if (url.field_set & (1 << UF_PATH)) {
            ...
            irc::sepstream pathstream(uri.substr(url.field_data[UF_PATH].off,
        ...
        if (url.field_set & (1 << UF_FRAGMENT)) {
            out.fragment = uri.substr(url.field_data[UF_FRAGMENT].off,
                                      url.field_data[UF_FRAGMENT].len);
        }

        std::string param_str;
        if (url.field_set & (1 << UF_QUERY)) {
            param_str = uri.substr(url.field_data[UF_QUERY].off,
        ...
        while (param_stream.GetToken(token)) {
            ...
            if (eq_pos == std::string::npos) {
                ...
                out.query_params.insert(std::make_pair(token.substr(0, eq_pos),
