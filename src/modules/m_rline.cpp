	...
	void Apply(User* u)
	{
		if (ZlineOnMatch)
		{
			...
			if (ServerInstance->XLines->AddLine(zl, NULL))
			{
				...
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline, "Z-line added due to R-line match on *@%s%s%s: %s",
...
class RLineFactory : public XLineFactory
{
 public:
	...
	XLine* Generate(time_t set_time, long duration, std::string source, std::string reason, std::string xline_specific_mask)
	{
		if (!rxfactory)
		{
			SnomaskManager::Write(SNO_LOCAL, SnomaskManager::announcement, "Cannot create regexes until engine is set to a loaded provider!");
 ...
 public:
	...
	CmdResult Handle (const std::vector<std::string>& parameters, User *user)
	{
		...
		if (parameters.size() >= 3)
		{
			...
			{
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::announcement,"Could not add RLINE: " + e.GetReason());
			}
			...
			if (r)
			{
				if (ServerInstance->XLines->AddLine(r, user))
				{
					if (!duration)
					{
						SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline,"%s added permanent R-line for %s: %s", user->nick.c_str(), parameters[0].c_str(), parameters[2].c_str());
					}
					else
					{
						time_t c_requires_crap = duration + ServerInstance->Time();
						std::string timestr = InspIRCd::TimeString(c_requires_crap);
						SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline, "%s added timed R-line for %s to expire on %s: %s", user->nick.c_str(), parameters[0].c_str(), timestr.c_str(), parameters[2].c_str());
					}
		...
		else
		{
			...
			{
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::xline,"%s removed R-line on %s",user->nick.c_str(),parameters[0].c_str());
			}
			else
			{
...
class ModuleRLine : public Module
{
 ...
 public:
	...
	void ReadConfig(ConfigStatus& status) CXX11_OVERRIDE
	{
		...
		{
			if (newrxengine.empty())
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::announcement, "WARNING: No regex engine loaded - R-Line functionality disabled until this is corrected.");
			else
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::announcement, "WARNING: Regex engine '%s' is not loaded - R-Line functionality disabled until this is corrected.", newrxengine.c_str());

			ServerInstance->XLines->DelAll(f.GetType());
		}
		else if ((!initing) && (rxfactory.operator->() != factory))
		{
			SnomaskManager::Write(SNO_LOCAL, SnomaskManager::announcement, "Regex engine has changed, removing all R-Lines");
			ServerInstance->XLines->DelAll(f.GetType());
		}
