 ...
 public:
	...
	void OnResult(const LDAPResult& r) CXX11_OVERRIDE
	{
		...
		for (std::vector<std::pair<std::string, std::string> >::const_iterator it = requiredattributes.begin(); it != requiredattributes.end(); ++it)
		{
			...
			{
				if (verbose)
					SnomaskManager::Write(SNO_LOCAL, SnomaskManager::connect, "Unable to compare attributes %s=%s: %s", attr.c_str(), val.c_str(), ex.GetReason().c_str());
			}
		}
		...
		{
			if (verbose)
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::connect, "Forbidden connection from %s (unable to validate attributes)", user->GetFullRealHost().c_str());
	...
	void OnError(const LDAPResult& err) CXX11_OVERRIDE
	{
		...
		{
			if (verbose)
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::connect, "Forbidden connection from %s (%s)", user->GetFullRealHost().c_str(), err.getError().c_str());
			ServerInstance->Users->QuitUser(user, killreason);
		}
...
public:
	...
	ModResult OnUserRegister(LocalUser* user) CXX11_OVERRIDE
	{
		...
		{
			if (verbose)
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::connect, "Forbidden connection from %s (No password provided)", user->GetFullRealHost().c_str());
			ServerInstance->Users->QuitUser(user, killreason);
			return MOD_RES_DENY;
		}
		...
		{
			if (verbose)
				SnomaskManager::Write(SNO_LOCAL, SnomaskManager::connect, "Forbidden connection from %s (Unable to find LDAP provider)", user->GetFullRealHost().c_str());
			ServerInstance->Users->QuitUser(user, killreason);
			return MOD_RES_DENY;
		}
