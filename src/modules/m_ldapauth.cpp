...
static std::string killreason;
static LocalIntExt *authed;
static bool verbose;
static std::string vhost;
static LocalStringExt *vhosts;
static std::vector<std::pair<std::string, std::string> > requiredattributes;

class BindInterface : public LDAPInterface
{
	std::string provider, uid, DN;
	bool checkingAttributes, passed;
	int attrCount;

	static std::string SafeReplace(const std::string &text, std::map<std::string,
 ...
 public:
	BindInterface(Module *c, const std::string &p, const std::string &u, const std::string &dn) : LDAPInterface(c), provider(p), uid(u), DN(dn)
	{
		checkingAttributes = false;
		passed = false;
		attrCount = 0;
	}
	...
	void OnResult(const LDAPResult &r) CXX11_OVERRIDE
	{
		...
		for (std::vector<std::pair<std::string, std::string> >::const_iterator it = requiredattributes.begin(); it != requiredattributes.end(); ++it)
		{
			...
			{
				if (verbose)
					ServerInstance->SNO->WriteToSnoMask('c', "Unable to compare attributes %s=%s: %s", attr.c_str(), val.c_str(), ex.GetReason());
			}
		}
...
class SearchInterface : public LDAPInterface
{
 ...
 public:
	SearchInterface(Module *c, const std::string &p, const std::string &u) : LDAPInterface(c), provider(p), uid(u)
	{
	}

	void OnResult(const LDAPResult &r) CXX11_OVERRIDE
	{
		LocalUser* user = (LocalUser *) ServerInstance->FindUUID(uid);
		dynamic_reference<LDAPProvider> LDAP(me, provider);
