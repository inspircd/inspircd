	...
	void OnSetUserIP(LocalUser* u) CXX11_OVERRIDE
	{
		...
		if (i != connects.end())
		{
			...
			if (i->second >= threshold)
			{
				...
				SnomaskManager::Write(SNO_REMOTE | SNO_BROADCAST, SnomaskManager::xline,"Module m_connectban added Z:line on *@%s to expire on %s: Connect flooding",
					maskstr.c_str(), timestr.c_str());
				SnomaskManager::Write(SNO_REMOTE | SNO_BROADCAST, SnomaskManager::announcement, "Connect flooding from IP range %s (%d)", maskstr.c_str(), threshold);
				connects.erase(i);
			}
		}
		else
