...
bool irc::equals(const std::string& s1, const std::string& s2) {
    ...
    return (national_case_insensitive_map[*n1] ==
            national_case_insensitive_map[*n2]);
}
...
size_t irc::find(const std::string& haystack, const std::string& needle) {
    ...
    for (size_t hpos = 0; hpos <= haystack_last; ++hpos) {
        ...
        for (size_t npos = 0; npos < needle.length(); ++npos) {
            if (national_case_insensitive_map[(unsigned char)needle[npos]] !=
                    national_case_insensitive_map[(unsigned char)haystack[hpos + npos]]) {
...
bool irc::insensitive_swo::operator()(const std::string& a,
...
size_t irc::insensitive::operator()(const std::string &s) const {
     ...
     */
    size_t t = 0;
    for (std::string::const_iterator x = s.begin(); x != s.end();
            ++x) { /* ++x not x++, as its faster */
...
irc::sepstream::sepstream(const std::string& source, char separator,
                          bool allowempty)
...
irc::portparser::portparser(const std::string &source, bool allow_overlapped)
    : sep(source), in_range(0), range_begin(0), range_end(0),
...
long irc::portparser::GetToken() {
    ...
    if (dash != std::string::npos) {
        ...
        if ((range_begin > 0) && (range_end > 0) && (range_begin < 65536)
                && (range_end < 65536) && (range_begin < range_end)) {
